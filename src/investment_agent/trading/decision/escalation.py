...
논지가 없는 미보유 종목은 기대수익을 줄여 담는다(`AlphaPolicy.unverified_entry_scale`, 엄격 모드
`require_verified_entry`면 막는다). Deep LLM을 건너뛰면 비용이 줄어드는 것이 아니라 그 종목이 그날 LLM 거부권
없이, 줄어든 비중으로 들어간다. 그래서 건너뛸 수 있는 경우는
