...
def evaluate(
    ...
    *,
    ...
    if last_filed is None:
        checks.append(_check("collection_stalled", ERROR, "financials가 비어 있다"))
    else:
    ...
    if latest_period is None:
        checks.append(_check("financials_stale", ERROR, "financials가 비어 있다"))
    else:
        age = (today - latest_period).days
        severity = ERROR if age > stale_mv_days else OK
        checks.append(_check(
            "financials_stale", severity,
            f"financials 최신 분기가 {age}일 전이다",
