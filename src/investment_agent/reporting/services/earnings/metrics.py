...
def eps_diluted(row: dict) -> float | None:
    """분기 희석 EPS. 저장된 `eps_diluted_gaap`만 쓴다.

    카드에서 순이익 ÷ 희석주식수로 다시 계산하지 않는다. 보고값이 없는 분기(대개 Q4)는 적재 단계가
    주식수 단위 검사를 거쳐 이미 계산해 두었고, 거기서도 만들지 못했으면 빈칸이 틀린 숫자보다 낫다
    (재계산하면 MCD가 3,321,614.40, UNH가 0.07처럼 나갔다).
    """
