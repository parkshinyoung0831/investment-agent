...
def _trace_steps(trace: Mapping[str, Any], ticker: str) -> list[dict[str, str]]:
    ...
    return [
        {"title": "후보 선별", "value": f"#{candidate.get('rank_position')} · {display_percent(candidate.get('rank_score'))}" if candidate else "기록 없음", "detail": f"{candidate.get('domain_count') or 0}개 근거 영역" if candidate else "후보 선별 결과가 없어요.", "status": "생성됨" if candidate else "미생성"},
        {"title": "이벤트 Feature", "value": f"중요도 {display_percent(feature.get('event_importance'))}" if feature else "기록 없음", "detail": f"이벤트 {feature.get('event_count') or 0}건 · 고영향 {feature.get('high_impact_event_count') or 0}건" if feature else "이벤트 snapshot이 없어요.", "status": "생성됨" if feature else "미생성"},
