...
def _intent(weights: dict[str, float]) -> ExecutionIntent:
    return ExecutionIntent(
        intent_id="intent-slice", risk_decision_id="risk-slice", proposal_id="proposal-slice",
...
class SellSlicingTest(unittest.TestCase):
    ...
    def test_a_large_exit_is_split_into_orders_within_the_limit(self):
        plans = self.plan({"AAPL": 0.0, "CASH": 1.0}, {"AAPL": 60.0}, {"AAPL": 200.0}, 12_000.0)
        self.assertEqual([plan.quantity for plan in plans], [25.0, 25.0, 10.0])
        self.assertTrue(all(plan.notional <= 5_000 for plan in plans))
        self.assertEqual(len({plan.client_order_id for plan in plans}), 3)
        again = self.plan({"AAPL": 0.0, "CASH": 1.0}, {"AAPL": 60.0}, {"AAPL": 200.0}, 12_000.0)
        self.assertEqual(plans, again)

    def test_unsplit_orders_keep_their_existing_idempotency_key(self):
        self.assertEqual(
    ...
    def test_total_notional_limit_still_applies_to_split_sells(self):
