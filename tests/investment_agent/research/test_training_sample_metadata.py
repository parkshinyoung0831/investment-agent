...
class TrainingSampleMetadataTest(unittest.TestCase):
    def test_scalar_projection_filters_ticker_window_and_label_cutoff(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            store.upsert_records("rl_feature_snapshots", [
                {
                    "record_key": f"{point}:AAA", "ticker": "AAA", "as_of_at": point,
                    "input_hash": "aaa-hash",
                    "features": {"large_payload": [1, 2, 3]},
                },
                {
                    "record_key": f"{point}:BBB", "ticker": "BBB", "as_of_at": point,
                    "input_hash": "bbb-hash",
                },
                {
                    "record_key": f"{later_point}:AAA", "ticker": "AAA", "as_of_at": later_point,
                    "input_hash": "late-hash",
                },
                {
                    "record_key": "2025-12-31:AAA", "ticker": "AAA",
                    "as_of_at": "2025-12-31T00:00:00+00:00",
                    "input_hash": "outside-window",
                },
            ], key="record_key")
            ...
            with patch.object(store, "records", side_effect=AssertionError("full payload read")):
                result = store.training_sample_period_inputs(
                    ...
                    end_as_of="2026-01-05T00:00:00+00:00",
