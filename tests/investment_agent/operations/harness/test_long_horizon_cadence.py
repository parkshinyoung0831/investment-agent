"""중장기 보유 전략의 실행 주기: 분석·진입 감시가 분 단위 매매 주기로 되돌아가지 않는다."""
...
from investment_agent.operations.harness.pipeline import entry_watch_job


class LongHorizonCadenceTest(unittest.TestCase):
    def test_entry_watch_runs_every_five_minutes_by_default(self):
        job = entry_watch_job(watch=lambda context: None)
        self.assertGreaterEqual(job.interval_seconds, 5 * 60)
