...
![ExecutionIntent 주문 실행 수명주기](diagrams/svg/execution-lifecycle.svg)

주문 하나가 지나는 상태와, 어느 전이가 되돌릴 수 없는지.

*소스: `docs/diagrams/src/execution-lifecycle.lifecycle.json` — 그림을 고치려면 이 파일을 고치고 `python scripts/build_diagrams.py`.*
...
![토스 실주문 실행 안전 런북 워크플로](diagrams/svg/execution-runbook.svg)
