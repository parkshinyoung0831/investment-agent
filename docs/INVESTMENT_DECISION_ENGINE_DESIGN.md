...
**우선순위**: balance_sheet category의 IC는 5·20·60일 모두 0 근처다(20일 +0.002, 생존 편향 축소 뒤). 결측 의미를
나누려면 feature 정의를 바꾸고 5년 feature를 전부 다시 적재해야 하는데, 기대 효과가 그 비용보다 작다고 보고
보류한다. 운영 영향은 무차입·자본잠식 종목이 이 category 없이 나머지로 채점되는 것이다.
...
**category 가중 challenger 검증** [실측, `artifacts/research/ablation/v6q`, 생존 편향 축소 뒤]: quality IC가 모든
기간 음수라 종합 점수에서 빼고 게이트로만 쓰는 변형(`quality_gate_only`)을 운영 기본값과 같은 PIT 데이터로
돌렸다. 초과수익 +13.6%p → +8.8%p, IR 0.158 → 0.083, 2022 MDD 19.9% → 22.2%로 **기각**이다. quality의 단독 IC가
음수여도 종합 점수 안에서는 낙폭을 줄인다 — 개별 IC만으로 category를 빼지 않는다.

**자기 역사 대비 밸류에이션** [실측, 225개 주간 시점, 20거래일]: 종목별 지난 156주 자기 분포 안의 백분위를
원값 횡단면 순위와 비교했다. 이익수익률 +0.016 → +0.019, FCF수익률 +0.000 → +0.015, PSR +0.016 → +0.005.
어느 차이도 유의하지 않다(t < 1). factor로 승격할 근거가 없어 LLM 검증 질문·맥락 정보 후보로만 남긴다.
...
**생존 편향을 줄인 뒤 재측정** [실측, `artifacts/research/ablation/v5`·`v5b`]: 위 표는 재현 시점 멤버의 최대 16%가
가격이 없어 빠진 표본이다(Master P0-15). 상장 중인 과거 멤버 50종목을 백필해 누락을 최대 8%로 줄인 뒤:

| 변형 | 초과수익 | IR | 평균 현금 | 2022 MDD |
|---|---|---|---|---|
| 절대 위험(이전 운영) | −48.6%p | −0.55 | 60.3% | 6.6% |
| SPY 대비 위험, 게이트가 최대 종목 수를 사후에 자름(`system-target-v4`) | +7.6%p | 0.06 | 15.0% | 20.6% |
| SPY 대비 위험 + optimizer가 최대 종목 수를 앎(`system-target-v5`, 운영) | **+13.5%p** | **0.16** | 13.5% | 20.2% |

v5가 채택 기준표를 모두 통과한다. 게이트의 최대 종목 수 한도는 5년 목표의 62%에서 사후에 작은 비중을 현금으로
돌렸다 — 상위 종목만으로 다시 풀면 같은 노출이 한도 안에서 재배분된다. factor IC도 약간 오른다(composite 20일
0.014 → 0.021, quality는 여전히 음수).
...
**검증 상태**: 재현에는 과거 논지·채택 ML이 없어 방향 근거가 factor 하나뿐이고 confidence가 항상 1이다 —
이 변경의 효과를 재현으로 잴 수 없다. 운영 논지가 쌓여 confidence가 1이 아닌 목표가 생긴 뒤 단계 진단
(`stage_diagnosis`)의 논지 증분과 함께 판정한다.
