...
> **구성**: Part I(§1~§19) 수치 엔진, Part II(§41~§58) AI 판단 계층과 토큰 비용. §20~§40은 비워 둔다 — Part II 번호를 코드 주석(`llm/usage.py`, `analysis.py`의 §50)이 참조하므로 바꾸지 않는다.
...
| router(`decide_escalation`) | 복원하되 **shadow 전용**: 판정만 기록하고 아무것도 건너뛰지 않는다 | 건너뛰기는 편입 게이트를 바꾼다(§43) |
...
- **역할별 reasoning_effort·verbosity**: 분석가 5명은 사실 요약이다. `reasoning_effort=low`(또는 minimal), `verbosity=low`의 후보. 절감 크기는 **reasoning 토큰을 계측하기 전에는 모른다.** 계측 필요.
...
| 2 | 캐시용 메시지 재배치 | 하한의 ~19% | 낮음(프롬프트 순서 변경) | 작다 | **지금 구현, 계측으로 적중 확인** |
...
## 57. 문서 확정 뒤 구현 계획 (승인 대기)

각 단계는 독립 커밋이고 `git revert` 한 번으로 되돌릴 수 있다. 하네스·판단 코드를 만지므로 매 단계 전에 정비 보류를 건다.
