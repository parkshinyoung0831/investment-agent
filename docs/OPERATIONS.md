...
이름을 받지 못한 종목은 시도 시각을 저장하지 않으므로 실행할 때마다 다시 조회한다(재시도 간격 옵션 없음).
...
![SEC 실적 공시 감지 및 2단계 알림 시퀀스](diagrams/svg/earnings-pipeline.svg)

8-K 속보와 10-Q/K 정밀 카드가 각각 언제 어떤 경로로 나가는지.

*소스: `docs/diagrams/src/earnings-pipeline.sequence.json` — 그림을 고치려면 이 파일을 고치고 `python scripts/build_diagrams.py`.*
