...
**재적재 결과** (`backfill_research_history --start 2021-09-03 --every-days 7`, 2026-09-21): 저장소 정리 뒤 259개 시점을 새 정의로
처음부터 다시 만들었다(약 85분, 오류·경고 0건). **로그가 아니라 Parquet를 직접 세어 확인했다** — feature snapshot 119,546건(259개 시점,
2021-09-03 ~ 2026-08-14), forward label 118,697건(20거래일 SPY 초과수익, 가격이 없는 849건 제외), 학습 표본 118,697건,
밸류에이션 관측 130,352건. snapshot 표에 `feature_version` 컬럼이 없음을 확인했다. 학습 표본 통계(왕복 비용 0.2% 차감 순초과수익 평균
−0.39%, 비용으로 부호가 뒤집힌 표본 1,496건)는 이전 세대와 같다 — 표본 통계는 feature 값이 아니라 label·비용에서 나오기 때문이다.
ML 후보 모델은 지운 뒤 다시 학습하지 않았다: champion이 없으면 `ml_serving`은 "no adopted model"로 예측을 내지 않고(정상 경로),
채택은 사용자 승인 원칙이다.
