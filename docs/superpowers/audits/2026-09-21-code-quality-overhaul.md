...
**하지 않은 것**: EX2-17의 (a)~(g)는 실주문 원장 쓰기 의미를 바꾸는 항목이라 고치지 않고 남긴다. 은행·리츠 매출 합산(총계 태그가
없는 은행의 "이자수익+비이자수익" 합성 규칙)은 여전히 하지 않는다 — 얻는 것은 P/S가 의미 없는 은행 몇 종목의 매출뿐이라
우선순위가 낮다. 총계 태그가 없으면 재처리 뒤에도 매출은 빈칸이 정상이다(틀린 값보다 빈칸).

**2026-09-22 갱신**: `revenue` 태그 우선순위 자체(총계 우선)는 이미 코드에 있다(FI-1/FI-2, 위 9.1절). 이걸 기존
~19,000개 공시에 반영하려면 재처리가 필요한데, 버전을 계속 올리는(v2→v3→…) 대신 재처리가 끝난 세대를
다시 "v1"이라 부르기로 사용자가 정했다(9.7절 이후, [[revenue-tag-priority-total-before-606]] 메모리 참고).
`SEMANTIC_POLICY_VERSION`은 코드에서 이미 `"v1"`이다 — 남은 건 `backfill_history --content company --scope all`
실행뿐이고, `reconcile_wide_history`가 하드 DELETE를 하므로 이번에도 사용자가 직접 돌린다.
...
- **은행·리츠 매출 재처리**: 태그 우선순위 수정은 코드에 반영됐고 버전 라벨도 "v1"로 확정됐다(사용자 결정, 2026-09-22).
  남은 건 `backfill_history --content company --scope all` 실행뿐이며, `reconcile_wide_history`의 하드 DELETE
  때문에 사용자가 직접 돌린다. 완료 뒤에는 revenue를 쓰는 research feature도 재적재가 필요하다
  ([[feature-version-removed-2026-09-21]]과 같은 절차).
