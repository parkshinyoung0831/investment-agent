...
위 그림이 **패키지가 무엇을 알아도 되는가**를 말한다면, 아래는 **공개 데이터 한 줄이 주문이
되기까지 실제로 무엇을 지나는가**를 단계별로 편 것이다. 열 개 띠가 순서대로 한 단계씩이고,
같은 띠 안에서도 네모의 크기가 다른 것은 역할의 크기가 다르기 때문이다.

![수집에서 자동매매까지 단계별 전체 지도](diagrams/svg/end-to-end.svg)

**이 그림의 화살표는 전부 코드에 있는 것만 그렸다.** 같은 띠에 나란히 있어도 서로 부르지
않으면 잇지 않았다 — 예를 들어 `팩터 5종`과 `ML Champion`은 둘 다 `FeatureLayer`를 읽는
형제이지 순서가 아니고, 저장소 넷도 서로를 부르지 않는다.

| 띠 | 이 단계에서 결정되는 것 |
|---|---|
| ① 외부 원천 | 공개 데이터만 쓴다. 유료 피드도, 사설 시그널도 없다 |
| ② 수집 실행 | Actions cron이 정기분을, 고정 IP 로컬 하네스가 뉴스·소셜·실행을 맡는다. 외부 호출은 **쓰기 전에** 일일 cap 슬롯을 예약한다 |
| ③ 저장 | 저장소가 넷이고 성격이 다르다 — 공용 원천은 Supabase, 원문·연구 파생·실행 원장은 로컬 |
| ④ 연구 | `PitReader`가 cutoff 이전만 읽고, 그 위에서 feature·팩터·ML이 만들어진다 |
| ⑤ 후보 · 판단 | tracked 범위 안에서 **분석 순서**를 정하고(매수 점수가 아니다), 근거를 묶어 LLM 토론에 넘긴다 |
| ⑥ 위험 · 최적화 | 비중은 optimizer가 내고, 절대 한도는 `RiskGate`가 다시 강제한다. LLM은 여기에 관여하지 않는다 |
| ⑦ System · My | System 목표는 실계좌·승인 결과를 **읽지 않는다.** 실계좌 추종은 My Portfolio 쪽에서만 제안된다 |
| ⑧ 승인 · 체결 | 빨간 경계 안쪽. 사람이 승인한 의도만, **보내기 전에** 원장에 예약하고 나서 나간다 |
| ⑨ 성과 · 피드백 | 체결 사실이 성과가 되고, 20일 뒤 판단이 채점되어 모델 승격 판정으로 돌아온다 |
| ⑩ 읽기 · 표시 | 화면과 알림은 저장소를 열지 않고 `reporting`만 읽는다. 알림은 보내기 전에 원장이 선점한다 |

*소스: `docs/diagrams/src/end-to-end.architecture.json` — 그림을 고치려면 이 파일을 고치고 `python scripts/build_diagrams.py`.*
