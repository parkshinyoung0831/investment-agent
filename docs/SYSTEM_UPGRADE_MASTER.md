...
### P0-14 운영 System이 목표를 한 번도 만들지 못했다

- **상태**: `FIXED` (`8316fa6`, `c9e2f55`)
- 후보 중 가격 이력이 7일뿐인 종목(BE·ILMN 신규 편입, PSTG→P 개명)이 공분산 겹침 구간을 6일로 줄였다(< 60).
  `market_backfill --scope missing`이 "행이 없는 종목"만 받았고, Actions 지연으로 일일 적재가 먼저 며칠을 넣어
  백필에서 빠졌다. 백필 대상을 "1년 전 가격 없음"으로 바꾸고, 가격 창이 부족한 신규 후보는 목표에서 뺀다.

### P0-15 과거 재현에 생존 편향이 있었다

- **상태**: `PARTIAL` (`54d5514`)
- 재현 시점 S&P 멤버 중 가격이 없어 빠진 종목이 2021-09에 505 중 82였다. 가격 적재가 추적 종목만 받았고,
  로컬 사본은 과거 멤버십이 가리키는 자리표시 증권의 가격만 복사했다. 상장 중인 50종목을 백필하고 사본·feature를
  다시 만들고, 옛 ticker로 남은 개명 종목(FLT→CPAY·GPS→GAP·FBHS→FBIN·DISCA/DISCK→WBD·WLTW→WTW)을 코드
  catalog(`data/universe/domain/ticker_renames.py`)로 현재 표기에 이어 재현 시점의 최대 누락이 6.9%가 됐다. 남은
  것은 인수·상장폐지 종목(공급자에 이력 없음)이라 승격 증거의 생존 편향 허용치(5%)를 넘는 것으로 기록된다.
- 이 날 이전의 재현·factor IC 수치는 누락 16%인 표본으로 잰 것이다.

### P0-16 승격 증거를 만드는 곳이 없어 실계좌 추종이 영구히 막혀 있었다

- **상태**: `FIXED` (`b33fdb7`)
- 수동 승격 게이트와 실주문 직전 검사는 artifact별 `portfolio_evaluations`만 읽는데 쓰는 곳이 없었다.
  `system_evaluations`(하네스 주간)가 재현과 운영 NAV로 그 행을 만든다. 게이트 기준(연 turnover 2.0, 모든 창
  초과수익 양수, MDD 20%, 생존 편향 5%)은 그대로다 — 기준을 넘지 못하면 승격되지 않는 것이 맞다.

### P0-17 종목당 LLM 비용에 역할 토론 호출이 빠져 있었다

- **상태**: `FIXED` (`b964a77`)
- runner가 역할 호출용 client의 사용량을 버려 종목당 구조화 1건(약 13K 입력)만 기록됐다. 그 전의 역할별
  비용 수치는 역할 호출이 빠진 값이다.
