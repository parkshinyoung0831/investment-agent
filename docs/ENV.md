...
| `AI_INVESTOR_TYPESAFE_API_KEY`, `AI_INVESTOR_TYPESAFE_BASE_URL`, `AI_INVESTOR_TYPESAFE_MODEL`, `AI_INVESTOR_TYPESAFE_TIMEOUT_SEC` | System-One(Jev) shadow provider. **키가 없으면 그 provider는 호출되지 않는다** — 채택이 아니라 비교 레일이다 (`INVESTMENT_DECISION_ENGINE_DESIGN.md` §49.2) |
