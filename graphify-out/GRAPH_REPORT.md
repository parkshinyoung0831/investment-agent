...
## Corpus Check
- 1196 files · ~923,634 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 15931 nodes · 34700 edges · 709 communities (562 shown, 110 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 1217 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a3383199`
...
- institutional/domain/__init__.py
- market_schedule.py
- SurpriseRowsTest
...
- earnings_calendar/candidates.py
...
- src/investment_agent/data/macro/releases/__init__.py
...
- date
- StorageLayoutTest
- src/investment_agent/data/market/__init__.py
- 45_performance.sql
- DuckDBOpenRetryTest
- graphify reference: GitHub clone and cross-repo merge
- graphify reference: transcribe video and audio
...
- src/investment_agent/trading/decision/__init__.py
- SurpriseViewContractTest
- AdoptedWeightsTest
- KillSwitchTest
...
- .current_model_stage
- .latest_adopted_weights
- src/investment_agent/trading/system/__init__.py
- .test_every_function_the_loaders_call_is_allowlisted
- tests/investment_agent/trading/system/__init__.py

## God Nodes (most connected - your core abstractions)
1. `parse_datetime()` - 264 edges
2. `get_logger()` - 178 edges
3. `canonical_json()` - 167 edges
4. `FakeDatabase` - 165 edges
5. `SupabaseRepository` - 158 edges
6. `ContractError` - 122 edges
7. `ExecutionSafetyError` - 121 edges
8. `Database` - 107 edges
9. `load_config()` - 96 edges
10. `ExecutionRepository` - 96 edges

## Surprising Connections (you probably didn't know these)
- `interactive_menu()` --calls--> `run_control_center()`  [INFERRED]
  launcher.py → src/investment_agent/operations/control_center.py
- `main()` --calls--> `run_control_center()`  [INFERRED]
  launcher.py → src/investment_agent/operations/control_center.py
- `_runs_per_month()` --calls--> `_count()`  [INFERRED]
  scripts/actions_budget.py → src/investment_agent/notifications/db.py
- `run()` --calls--> `PostgresNotificationLedger`  [INFERRED]
...
## Communities (709 total, 110 thin omitted)

### Community 0 - "web.py"
Cohesion: 0.12
Nodes (36): _cboe_put_call(), _cboe_put_call_on(), _cboe_vix(), _cnn_fear_greed(), fetch_batch(), _get(), _month_starts(), _multpl() (+28 more)

### Community 1 - "build_research_dataset"
Cohesion: 0.08
Nodes (36): DatasetManifest, FeatureRecord, _finite_features(), LabelRecord, _period(), Any, 연구 산출물의 provenance와 PIT 경계를 표현하는 계약., dataset hash·기간·PIT cutoff을 모델 artifact와 함께 추적한다. (+28 more)

### Community 2 - "quant.py"
Cohesion: 0.08
Nodes (57): _allocation_items(), _allocation_summary(), _allocation_text(), _asset_label(), _assets_for_allocations(), _comparison_chart(), _is_equal_weight_allocation(), _metric_row() (+49 more)

### Community 3 - "EvidenceBundle"
Cohesion: 0.09
Nodes (43): EvidenceBundle, _date_ok(), _domain_payload(), fetch_fundamentals(), fetch_macro_indicators(), fetch_statement(), TradingAgents Fundamentals Analyst를 위한 SEC 재무제표·Gurus·거시지표 데이터 소스 어댑터., 재무제표, 밸류에이션, 세그먼트, 13F 기관 대가 지분을 시점 일치 번들에서 읽는다. (+35 more)

### Community 4 - "SupabaseRepository"
Cohesion: 0.03
Nodes (65): normalize_ticker(), universe의 Yahoo식 표기(BRK-B)에 맞춘다., SecurityProposal의 생성 배치와 유효기간을 묶는다., SignalRecord, _feature_snapshot(), _finite(), _guru_candidate_signals(), _iso() (+57 more)

### Community 5 - "live_worker.py"
Cohesion: 0.04
Nodes (79): LiveExecutionPermit, 소비된 승인 하나가 허용하는 주문 집합. **몇 분짜리다.**, 저장소 표기(BRK-B)를 토스 표기(BRK.B)로 변환한다., to_toss_symbol(), TossUsRegularSession, _decimal(), _decimal_text(), parse_personal_order_event() (+71 more)

### Community 6 - "macro/core.py"
Cohesion: 0.04
Nodes (79): 발표 예정 카드 렌더 — HTML 템플릿 채우기 + 헤드리스 브라우저로 PNG 캡처. 알림 패키지는 자기 render와 templates를…, Any, 실적 카드 렌더 — HTML 템플릿 채우기 + 헤드리스 브라우저로 PNG 캡처. macro/render.py와 같은 경로지만 펀더멘탈 전용…, 펀더멘탈 템플릿에 ctx를 채워 HTML 문자열을 반환., HTML을 브라우저로 열어 PNG로 캡처하고 파일 경로를 반환., render(), shoot_png(), _build_ctx() (+71 more)

### Community 7 - "ExecutionIntent"
Cohesion: 0.06
Nodes (38): ExecutionIntent, 현금을 뺀 대상 종목. 주문 계획이 도는 목록이다., 승인 뒤 계좌·시세가 주문 수량을 바꾸지 않았는지 다시 계산한다., revalidate_live_handoff(), client_order_id(), ExecutionLimits, OrderPlan, datetime (+30 more)
...
### Community 9 - "canonical_json"
Cohesion: 0.11
Nodes (13): canonical_json(), 같은 내용이면 항상 같은 문자열. 해시와 중복 판정의 기준이다., decision_features(), decision_inference_frame(), decision_training_set(), 저장된 원본 판단을 관측 상태로 쓰는 가상 시장 PPO 학습 입력., 실제 원본 제안의 scalar만 같은 순서의 관측값으로 변환한다., CanonicalJsonTest (+5 more)
...
Cohesion: 0.03
Nodes (137): _frame_from_prices(), _latest(), Any, DataFrame, 저장된 AI 투자 판단을 선택한 깊이만큼 읽기 전용으로 검토한다., 선택된 가격 뷰 안에서만 저장된 OHLCV를 읽고 지표를 계산한다., 주문·체결 페이지가 탭 하나로 불러 쓴다. 페이지 단독 진입점은 아니다. 스크립트였을 때의 st.stop()은 return으로 바꿨다 — 탭…, render() (+129 more)
...
Cohesion: 0.16
Nodes (25): clear_screen(), configure_console(), developer_tools_menu(), find_available_port(), harness_switch_command(), interactive_menu(), is_port_available(), main() (+17 more)

### Community 14 - "watch_earnings.py"
Cohesion: 0.08
Nodes (27): main(), main(), note_problem(), Problem, 사람이 받지 못했거나 받았는지 모르는 알림을 프로세스 단위로 모은다. 발송 코드는 결과를 모아 올리지 않고 각자 버리는 경우가 많다. 진입점이…, 알림 하나의 문제. key는 topic 안의 정체성(subject·occurrence)이다., 이 프로세스에서 아직 보고되지 않은 문제를 꺼내고 비운다., 남은 문제를 로그로 남기고 개수를 돌려준다. 0이 아니면 진입점은 실패로 끝낸다. (+19 more)

### Community 15 - "investment_harness.py"
Cohesion: 0.11
Nodes (39): build_registry(), _emit(), main(), 항상 켜진 로컬 장비용 투자 분석 운영 하네스. 인자 없이 실행하면 상태 파일이나 외부 서비스에 손대지 않고 계획만 출력한다., JobDefinition, StageDefinition, 고정 IP 로컬 장비에서 분석·승인 단계를 감시하는 운영 하네스., default_job_kill_env() (+31 more)

### Community 16 - "logging.py"
Cohesion: 0.02
Nodes (156): 프로세스로 시작한 CLI만 거치는 자리. 여기서 두 가지를 한다 — 로컬 `.env`를 읽는 것과, 루트 로거를 JSON 한 줄로 맞추는 것.…, 프로세스 진입점 준비. `__main__` 블록에서만 부른다., start_cli(), main(), _parse_args(), Namespace, Yahoo 발표 이력으로 EPS 예상치를 재구성해 저장하는 수동 백필 진입점., 명시적으로만 수행하는 역사 EPS 예상치 백필. (+148 more)

### Community 17 - "parse_datetime"
Cohesion: 0.03
Nodes (66): Any, 상태와 결정이 어긋나면 거절한다. 어긋난 행을 통과시키면 "승인되지 않았는데 승인된 것처럼 보이는" 상태가 생기고, 그 상태를 만든 경로는…, sha256_field(), snowflake(), ExecutionSafetyError, RuntimeError, 주문을 만들지 않고 즉시 중단해야 하는 안전 오류., _approval() (+58 more)

### Community 18 - "sec.py"
Cohesion: 0.08
Nodes (35): _get_text_optional(), archive_cik(), filing_archive_base(), filing_archive_items(), filing_document_url(), filing_homepage_url(), filings_filed_since(), get_bytes_optional() (+27 more)

### Community 19 - "_render_discord_full"
Cohesion: 0.06
Nodes (65): 실제 Discord 실적 카드 근거를 선택한 한 섹션씩만 표시한다., _render_discord_full(), _render_trust_notes(), _big(), build(), _period_text(), 실적 PNG 카드 — 템플릿 컨텍스트(ctx)와 Discord 캡션 조립. candidates.load_pending() 항목 +…, 항목(+extras) → (템플릿 ctx, Discord 캡션). (+57 more)

### Community 20 - "serialization.py"
Cohesion: 0.06
Nodes (50): JSON 직렬화·해싱·URL 정규화. ## 왜 표준 json으로 부족한가 `json.dumps`는 `Decimal`과 `date`를 못…, BaselinePolicyConfig, BaselinePolicyModel, DurablePolicyArtifact, load_baseline_policy(), Any, ndarray, Path (+42 more)

### Community 21 - "market_risk.py"
Cohesion: 0.08
Nodes (28): _aligned_returns(), calculate_market_covariance(), calculate_market_risk(), _close_by_date(), estimate_betas(), estimate_trading_costs(), historical_tail_losses(), ledoit_wolf_constant_correlation() (+20 more)
...
### Community 23 - "domain/services/__init__.py"
Cohesion: 0.05
Nodes (29): backfill_historical_eps_estimates(), Any, ExpectationsRepository, 기존 실적 속보에만 Yahoo의 과거 EPS 예상치를 재구성해 저장한다., build_historical_eps_estimates(), _finite_number(), HistoricalEpsEstimateBatch, Any (+21 more)
...
Cohesion: 0.08
Nodes (34): RuntimeError, 뉴스 provider가 data 계층에서 반환하는 결과 계약., Reddit 자격증명이 없다. 오류가 아니라 아직 켜지지 않은 상태다., 오늘 provider 한도를 다 썼다. 수집 유스케이스와 reddit 어댑터가 함께 쓰는 계약이라 둘 중 어느 계층에도 두지 않는다.…, SocialCredentialsMissing, SocialQuotaExhausted, credentials_available(), fetch_new_posts() (+26 more)
...
Cohesion: 0.10
Nodes (36): default_database_path(), parquet_root(), Path, Intelligence의 DuckDB catalog와 Parquet archive 위치. 표 이름을 문자열 리터럴로 흩뿌리면 이름을 바꿀 때…, 이번 실행이 쓸 Intelligence DB 경로., 본문 Parquet 루트. 기본 production 경로는 명시적으로 고정한다. 테스트나 별도 profile에서 DuckDB 경로를 주면 그…, connect(), ddl_statements() (+28 more)

### Community 29 - "continuous_retrain.py"
Cohesion: 0.09
Nodes (30): adopt_candidate(), default_spec(), main(), _parse_args(), Any, datetime, Namespace, Path (+22 more)

### Community 30 - "reconcile_toss.py"
Cohesion: 0.10
Nodes (19): ApprovalStatusClient, ApprovalStatusPublishSummary, ApprovalStatusRepository, publish_reconciliation_statuses(), Protocol, 재조정 결과를 원래 Discord 승인 카드에만 best-effort로 되돌린다., 허용한 broker 축약 상태만 사람용 문구로 바꾼다., 원장 변경 뒤 exact approval/message에만 PATCH하며 실패를 전파하지 않는다. 승인봇 설정이 아직 없으면 조용히… (+11 more)

### Community 31 - "TransactionCostModel"
Cohesion: 0.08
Nodes (32): build_training_samples(), main(), _parse_args(), Any, datetime, Namespace, 확정된 feature/label을 **비용 반영 학습 표본**으로 바꿔 원장에 쌓는다. 이 entry가 채점과 학습 사이의 다리다.…, label이 확정된 (종목, 시점)마다 비용 반영 학습 표본을 하나씩 만든다. (+24 more)

### Community 32 - "select_all_paged"
Cohesion: 0.07
Nodes (48): ciks_for_tickers(), ciks_missing_financials(), 현재 추적 security에 연결됐지만 core fundamentals가 없는 CIK., Return the entity gate for SEC CompanyFacts work exactly once per CIK., Resolve ticker CLI selectors to their current CIKs without fan-out., tracked_ciks(), _filings_for(), fiscal_periods() (+40 more)
...
### Community 34 - "notify_ops"
Cohesion: 0.08
Nodes (42): main(), main(), _parse_args(), Namespace, _prune_history_without_failing_ingest(), date, 보존 정리는 실패해도 그날의 수집 결과를 되돌리지 않는다. 정리는 이미 저장한 가격을 다듬는 뒷정리이고, 가격 수집 결과와 독립적으로 재시도할…, _integer() (+34 more)

### Community 35 - "MarketRepository"
Cohesion: 0.05
Nodes (29): configure(), Database, MarketRepository, Any, Database, date, datetime, market 스키마를 읽고 쓰는 유일한 자리. ## 시장 가격의 PIT 경계 일별 종가는 거래일 종료 뒤 공개된 사실이다. 로컬 적재 시각은… (+21 more)
...
### Community 37 - "app_pages/macro.py"
Cohesion: 0.04
Nodes (72): _allocation_text(), _close_prices(), _econ_table(), _expected_value(), _market_snapshot(), _market_value(), _mode_text(), _page_link() (+64 more)

### Community 38 - "ProductionInvestmentAdapters"
Cohesion: 0.10
Nodes (28): _metadata_id(), ProductionInvestmentAdapters, ID를 metadata로 넘기고 모든 주문 mutation은 execution entry에만 맡긴다., 따라갈 System 목표를 고른다. 같은 목표로는 한 번만 승인을 묻는다., 새 Toss 계좌 스냅샷으로 `System 목표 − 실제 계좌` 제안을 기록한다. 주문은 아직 없다., 발표 예정 시각 창에서 관심종목 공시를 훑고 알림을 바로 보낸다. 거래 창(risk_window)으로 막지 않는다 — 주문이 아니라 공시…, PIT 밸류에이션 관측값을 원장에 적재한다. live_shadow만 만든다 — 과거 시점은 가격 적재시각과 TTM vintage를 증명할 수…, tracked universe의 PIT feature snapshot을 ResearchStore에 적재한다. 주문이 아니라 데이터 생산이라… (+20 more)

### Community 39 - "target.py"
Cohesion: 0.06
Nodes (44): alpha_universe(), AlphaPlan, AlphaPolicy, expected_return_signals(), is_valid_view(), Any, datetime, ALPHA: 이 종목을 지금 얼마나 매력적으로 보는가. 출력은 종목마다 기대초과수익 하나와, 기대수익으로 표현할 수 없을 때만 강제 제약… (+36 more)
...
Cohesion: 0.14
Nodes (27): _clean_text(), cluster_contents(), _direction(), _event_type(), extract_events(), _jaccard(), normalize_content(), normalize_contents() (+19 more)

### Community 42 - "dashboard/db.py"
Cohesion: 0.03
Nodes (162): P, R, _active_watchlist_rows(), _as_datetime(), _attach_entity_profiles(), _canonical_financial_rows(), _canonical_price_rows(), _canonical_processing_rows() (+154 more)
...
Cohesion: 0.07
Nodes (25): ContractError, ValueError, 값이 계약을 어겼다. 부르는 쪽이 고쳐야 하는 오류다., Claim, InvestmentDecision, _number(), Any, LLM 입출력과 저장 행을 검증하는 엄격한 계약. (+17 more)

### Community 48 - "releases/db.py"
Cohesion: 0.11
Nodes (59): 미래 발표의 예상값을 매번 확인하고, 달라진 상태만 DB가 원자적으로 저장한다., snapshot_forecasts(), main(), split_event_key(), enrich_series(), measure_definitions(), Any, 고정 30개 지표의 수집기 설정. API 옵션·라이선스 설명은 사실표에 반복하지 않는다. (+51 more)

### Community 49 - "adjust"
Cohesion: 0.06
Nodes (33): adjust(), dividend_factors(), _previous_close(), date, 분할·배당으로 과거 가격을 조정한다. ## 왜 저장하지 않고 계산하는가 조정가를 저장하면 **분할이 하나 새로 들어올 때마다 과거 행 전체를…, 거래일마다 곱할 분할 계수. 분할 당일(`action_date`)의 가격은 이미 분할 후 가격이다. 따라서 조정 대상은 **그 전날까지**다.…, 거래일마다 곱할 배당 계수(총수익 기준). 배당락일 전날 종가를 기준으로 `1 - 배당/종가`를 누적한다. 종가를 모르면 그 배당은 건너뛴다…, 배당락일 **직전 거래일**의 종가. 휴장을 건너뛰어야 하므로 목록을 거슬러 찾는다. (+25 more)

### Community 50 - "_FakeClient"
Cohesion: 0.20
Nodes (7): _FakeClient, manager_cik/name/fund_name/is_active는 코드 설정이 SSOT이므로 `institutional.managers`를…, 관심종목 밖 행을 읽지 않고, PostgREST 상한에서 잘리지 않아야 한다., execution 관측은 이제 Supabase가 아니라 로컬 runtime.sqlite3 (reporting.local_runtime)가…, account_daily_snapshots는 안전한 롤업 필드만 갖는 표라 raw_snapshot이 구조적으로 들어갈 자리가 없다 —…, SelectOnlyGatewayTests, _uncached()

### Community 51 - "ReportingQueries"
Cohesion: 0.05
Nodes (24): load_released(), parse_time(), Any, datetime, 경제 발표 알림의 reporting reader 경계. 보낼지 말지는 원장(엔진)이 판단한다., 최근 first actual이 확인된 발표. 같은 발표를 여러 번 읽어도 원장이 한 번만 보낸다., Database, 화면과 알림에 같은 DataResult를 반환한다. 실패를 빈 결과로 숨기지 않는다. (+16 more)

### Community 52 - "rl/contracts.py"
Cohesion: 0.05
Nodes (62): 단일 horizon label을 구간 종료가 확정된 뒤에만 만든다. `label_available_at`을 넘기면 실제 적재 시각을 근거로…, 1/5/20 거래일 종료 뒤에만 label을 생성한다., ResearchStore의 PIT feature snapshot을 Qlib research workflow에만 연결한다., FeatureSnapshot, _finite(), ForwardReturnLabel, _hash(), MembershipSnapshot (+54 more)

### Community 53 - "ticker_label"
Cohesion: 0.12
Nodes (32): mom(), Momentum score display: signed percentage-points without a percent sign., ticker_label(), ticker_text(), _canary(), duel_adm(), duel_gem(), gate_haa_bal() (+24 more)

### Community 54 - "FakeRepo"
Cohesion: 0.13
Nodes (7): 보유수량 대사: 우리 체결로 설명되지 않는 변화와 외부 미체결은 신규 주문을 막는다., event(), FakeApi, FakeRepo, ReconciliationWorkerTest, remote(), TcaRecordingTest
...
Cohesion: 0.09
Nodes (19): ApprovalRequest, 특정 주문 계획 하나에만 유효한 건별 사람 승인., ApprovalRepository, Protocol, 향후 worker용 1회성 gate다. 여기서는 주문을 호출하지 않는다., _Repository, _config(), _DecisionRepository (+11 more)

### Community 57 - "TradingRepository"
Cohesion: 0.06
Nodes (18): Any, datetime, 승인 전 제안·거절 audit만 기록한다. 승인은 수동 게이트를 거친다., 현재 단계를 잠근 뒤 승인 audit만 추가한다., 이미 계산된 회차 상태를 v1 원장에 기록한다., 완전성 배치와 immutable 종목 신호를 함께 기록한다., 기록 시각이 창 안인 종목 의견 전부(만료 여부와 무관). System 논지는 만료가 아니라 판단 시점으로 유효를 가른다., 승인·매수 여부와 무관한 원본 완료 판단을 읽는다. (+10 more)

### Community 58 - "load_or_create_approval_secret"
Cohesion: 0.26
...
### Community 61 - "system/engine.py"
Cohesion: 0.05
Nodes (48): main(), advance(), DailyMark, first_session_after(), _gross(), performance_summary(), datetime, System Portfolio의 비중 기반 회계. 가상 현금·수량·주문·체결이 없다. 한 거래일을 넘기는 계산은 둘뿐이다. 1. **평가**:… (+40 more)

### Community 62 - "application/refresh_expectations.py"
Cohesion: 0.06
Nodes (30): build_earnings_estimates(), ConsensusBatch, date, 시장 원천의 상대 기간을 표준 회계기간 스냅샷으로 변환한다., 예상치 수집 한 번에서 만들어진 정규화 결과., 컨센서스와 발표 예정일을 같은 회계기간 키에 맞춘다., collect_expectations(), CollectionBudgetExceeded (+22 more)

### Community 63 - "factors.py"
Cohesion: 0.20
...
### Community 65 - "load_model"
Cohesion: 0.10
Nodes (19): _load_booster(), load_model(), LoadedModel, Any, ndarray, 저장된 ML artifact를 오늘의 feature에 적용해 수치 예측을 만든다. `fit_baseline`이 학습만 하고 끝나면 모델은…, 저장한 booster 원문을 되살린다. 원문이 없는 옛 artifact는 중요도만 있어 복원할 수 없다., artifact 상태에서 복원한 예측기와 그 신뢰도 근거다. (+11 more)
...
### Community 67 - "sqlite.py"
Cohesion: 0.08
Nodes (15): _apply_schema(), _columns(), _migrate_legacy_decision_tables(), _migrate_legacy_execution_tables(), Connection, RuntimeError, 로컬 실행 원장의 SQLite 연결 경계. 실제 주문·승인·알림 중복 방지는 네트워크 DB가 아니라 실행 컴퓨터의 단일 파일에 기록한다.…, 기존 주문을 새 원장으로 검증해 옮겨야 한다. (+7 more)

### Community 68 - "universe/persistence.py"
Cohesion: 0.09
Nodes (42): 추적 종목의 누락 한글명을 토스증권으로 보강한다. 토스 IP 허용목록에 등록된 로컬 환경에서만 별도 실행한다. 기존…, refresh_korean_names(), append_memberships(), apply_entity_results(), apply_toss_names(), configure(), _db(), _listing_payload() (+34 more)

### Community 69 - "calculations/__init__.py"
Cohesion: 0.04
Nodes (113): covariance_to_correlation(), finite_number(), parse_date_safe(), parse_datetime_safe(), Any, datetime, 대시보드 계산 기본 파싱·변환 유틸리티., DataFrame 또는 매핑 시퀀스를 복사된 레코드 목록으로 정규화한다. (+105 more)

### Community 70 - "toss/client.py"
Cohesion: 0.04
Nodes (65): PositionSnapshot, access_token(), _authorized_headers(), fetch_accounts(), fetch_buying_power(), fetch_exchange_rate(), fetch_holdings(), fetch_open_orders() (+57 more)

### Community 71 - "OptimizerPolicy"
Cohesion: 0.06
Nodes (36): build_fusion_policy_read_model(), build_optimizer_policy_read_model(), build_risk_policy_read_model(), Any, 투자 정책을 화면용 bounded read model로 투영한다., 현재 signal fusion 정책을 변경 없이 직렬화한다., 현재 optimizer 기본 정책을 변경 없이 직렬화한다., 현재 portfolio risk 기본 한도를 변경 없이 직렬화한다. (+28 more)
...
Cohesion: 0.06
Nodes (61): _date_ok(), fetch_external_global_news(), fetch_external_news(), TradingAgents News Analyst를 위한 실시간 뉴스(yfinance/Alpha Vantage) 데이터 소스 어댑터., 선택한 upstream news vendor의 필수 설정을 네트워크 호출 전에 검증한다., 종목별 실시간 뉴스를 공급자(yfinance / Alpha Vantage)에서 수집한다., 시장 전반의 실시간 글로벌 뉴스를 수집한다., _symbol_ok() (+53 more)

### Community 77 - "FakeRepository"
Cohesion: 0.05
Nodes (38): FakeRepository, plan_with_funding(), 지금 가진 현금으로 매수를 다 못 대면 이번 승인은 **매도만** 담는다. 아직 체결되지 않은 매도대금은 현금이 아니다(worker의 연쇄…, 토스 앱에서 사람이 확인할 주문 한 줄., S&P 500 미국주식 sleeve의 읽기 전용 계좌 스냅샷., _ticket(), TossManualSnapshot, TossManualTicket (+30 more)

### Community 78 - "features/db.py"
Cohesion: 0.07
Nodes (43): earliest_change_since(), manifest_path(), Path, Market 수집 변경 manifest. 가격 fact에 local ingestion timestamp를 중복 저장하지 않는다. feature…, record_change_dates(), main(), Daily technical indicators entrypoint., changed_indicators() (+35 more)
...
Cohesion: 0.08
Nodes (38): BacktestResult, BacktestSafetyError, CashEvent, CorporateActionApplication, FillEvent, MarketBar, NavPoint, OrderEvent (+30 more)
...
Cohesion: 0.12
Nodes (26): aggregate_company_share_history(), _clean_member_title(), drop_implausible_share_rows(), _is_preferred_or_derivative(), _normalize_class_key(), parse_common_shares_from_companyfacts(), parse_common_shares_from_xbrl_document(), _positive_share_count() (+18 more)

### Community 84 - "application/backfill_history.py"
Cohesion: 0.06
Nodes (51): backfill_company_history(), backfill_segment_history(), _company_backfill_targets(), _completed_segment_accessions(), prune_segment_history(), date, 기업 재무 이력을 명시적으로 백필한다. 기업 전체 재무는 daily와 같은 companyfacts 원천을 쓰고, 세그먼트는 차원 데이터가…, 선택 CIK에서 완료된 전역 고유 accession_no 집합. (+43 more)

### Community 85 - "DiscordChannel"
Cohesion: 0.05
Nodes (38): 켜짐/꺼짐 값. 모르는 값은 **꺼진 것으로 읽는다**(fail-closed). 안전 플래그가 오타 하나로 켜지면 안 된다. 그래서 참으로…, DeliveryRejected, DeliveryUnknown, DiscordChannel, fetch_forum_threads(), _file_name(), Any, RuntimeError (+30 more)

### Community 86 - "purged_row_splits"
Cohesion: 0.20
Nodes (10): _default_splits(), main(), _parse_args(), Namespace, 시점 경계로 60/20/20을 나누고 label이 겹치는 시점을 purge한다. 행 번호로 자르면 같은 날짜의 종목이 train과…, dataset_periods(), purged_row_splits(), as_of 시점마다 (as_of, 최대 forward_end, 시작행, 끝행)을 만든다. `build_research_dataset`이 행을… (+2 more)

### Community 87 - "HarnessScheduler"
Cohesion: 0.07
Nodes (18): KillSwitches, 알 수 없는 값은 안전하게 ON으로 해석한다., _switch(), HarnessScheduler, _idempotency_key(), datetime, 한 tick 안에서 성공 stage를 진행하고 waiting·오류·kill에서 멈춘다., _run_id() (+10 more)

### Community 88 - "build_labels.py"
Cohesion: 0.05
Nodes (46): main(), date, datetime, 과거 시점의 PIT 밸류에이션·feature를 주기적으로 쌓고, 끝난 구간에 label·학습 표본을 붙인다. python -m…, start부터 every_days 간격의 평일 판단 시각. 주말에 걸리면 직전 금요일로 당긴다., replay_dates(), main(), _parse_args() (+38 more)

### Community 89 - "build_market_regime"
Cohesion: 0.08
Nodes (28): build_live_regime_read_model(), Any, 시장 가격을 Dashboard용 regime read model로 투영한다., SPY·QQQ·VIX 최근 가격으로 저장하지 않는 시장 국면 미리보기를 계산한다., MarketRegime, 종목별 판단보다 먼저 공유되는 시장 환경 snapshot이다., build_market_regime(), _clamp() (+20 more)

### Community 90 - "risk/gate.py"
Cohesion: 0.07
Nodes (34): 내용 기반의 결정론적 식별자를 만든다., stable_id(), follow_weights(), FollowOutcome, FollowPolicy, order_gaps(), plan_follow(), Any (+26 more)

### Community 91 - "SegmentMetricRepository"
Cohesion: 0.05
Nodes (26): CompanyFilingSource, EarningsFilingSource, Any, date, Protocol, 기업 전체 재무 공시 탐색과 Company Facts 원천., 세그먼트 XBRL 공시 탐색과 instance 문서 원천., 세그먼트 재무용 SEC FSDS 원천. (+18 more)

### Community 92 - "Database"
Cohesion: 0.03
Nodes (109): profit_measure_label(), `profit_measure_kind`의 사람이 읽는 이름. 모르는 값은 일반 표기로 돌려준다., main(), Daily gurus 13F polling entrypoint., configure(), ETL 진입점이 v1 DB 객체를 주입한다., _cutoff(), filing_notice() (+101 more)

### Community 93 - "market/domain/models.py"
Cohesion: 0.12
Nodes (21): action_rows(), attach_security_ids(), changed_prices(), clean_price_row(), collect_prices(), CollectedPrices, 일봉·기업행위 수집의 공통 절차. daily와 backfill은 대상과 기간만 다르다. ## 종목 신원은 수집을 시작할 때 고정한다 요청할…, 새 키이거나 저장된 값과 다른 가격 행만 돌려준다. (+13 more)

### Community 94 - "decision/analysis.py"
Cohesion: 0.06
Nodes (36): analysis_limit(), _attempt_case(), _case_key(), main(), _ml_fused_proposals(), _model_pool_ledger_path(), _parse_args(), datetime (+28 more)

### Community 95 - "ensure_aware"
Cohesion: 0.06
Nodes (51): FetchNews, FetchPosts, 거시 지표의 정정(revision)을 다루는 규칙. ## 발표된 숫자는 나중에 바뀐다 GDP·고용처럼 중요한 지표일수록 여러 번 고쳐진다.…, collect_news(), _mention(), datetime, 관심종목 뉴스 수집 유스케이스. ## fetch를 주입받는다 provider 호출을 이 모듈이 직접 하면 테스트가 네트워크를 타야 한다.…, 조회해서 받은 기사의 언급. 등급은 결정론적이다. (+43 more)

### Community 96 - "supabase/segment_metrics.py"
Cohesion: 0.06
Nodes (41): accessions_by_source(), _compact_row(), compact_rows(), _company_metrics_for(), _dedupe(), delete_stale_metrics_for_accessions(), existing_accessions(), fetch_metric_years() (+33 more)

### Community 97 - "release_calendar.py"
Cohesion: 0.20
Nodes (19): _as_datetime(), _failure(), ingest_raw(), Any, date, datetime, 경제발표 일정·예상값·원자료 변경을 수집하는 ETL. actual은 DB 조회 계층이 계산한다., ISO UTC/aware datetime와 datetime 객체를 동일하게 비교한다. (+11 more)
...
Cohesion: 0.03
Nodes (171): Renderer, Config, default_config(), load_config(), Path, 설정을 명시적으로 읽는 자리. 실제 환경변수가 `.env`보다 우선하고, 필수 값은 호출 시점에 검증하며, 비밀값은 로그에 기록하지 않는다., 이 실행이 쓰는 설정. 값은 프로세스 환경에서 온다., 설정을 한 번 읽는다. **진입점에서만** 부른다. `use_dotenv=False`는 테스트용이다 — 로컬 `.env`가 테스트 결과를… (+163 more)
...
### Community 102 - "ContinuousLearner"
Cohesion: 0.10
Nodes (19): DeflatedSharpeRatio, DeflatedSharpeResult, _norm_cdf(), _norm_ppf(), Marcos Lopez de Prado 교수의 Deflated Sharpe Ratio (DSR) 과적합 검정 엔진. 다중 가설…, Deflated Sharpe Ratio 검정 결과., 수익률 시계열과 시도 횟수를 바탕으로 DSR 확률을 계산한다., ContinuousLearner (+11 more)

### Community 103 - "retry.py"
Cohesion: 0.07
Nodes (32): HTTPError, HTTPStatusError, _api_error(), fetch_batch(), _fetch_one(), _fmt(), _freq_token(), date (+24 more)
...
Cohesion: 0.08
Nodes (41): FilingErrorSink, ShadowSink, FilingRecord, Position, 13F 수집·저장 경계에서 사용하는 명시적 데이터 모델., RPC에 넘기는 원천 포지션 배열. DB 컬럼 계약과 1:1로 맞춘다., SEC 원본 한 행을 USD로 정규화한 DB 저장 단위. Combination Report의 중복 보유를 Python에서 먼저 합치면 sub-…, 검증을 마친 13F 공시와 합산 포지션 묶음. (+33 more)

### Community 109 - "TradingAgentsAdapterTest"
Cohesion: 0.07
Nodes (15): EvidenceItem, DecisionEngine, Protocol, AI Investor 판단 엔진 인터페이스 및 공통 경계., _bundle(), _Client, _ExternalClient, _ExternalRunner (+7 more)
...
Cohesion: 0.08
Nodes (22): canonicalize_url(), 같은 문서를 가리키는 URL을 한 모양으로. 읽을 수 없으면 `None`. 같은 기사에 tracking 파라미터만 달라진 URL이 붙어…, ExternalContent, LocalEvidenceCache, _normalized_content(), Any, datetime, Path (+14 more)
...
### Community 113 - "UniverseRepository"
Cohesion: 0.04
Nodes (62): MembershipSnapshot, normalize_ticker(), 밖에서 온 종목 표기를 우리 표기로. 읽을 수 없으면 `None`. 점을 하이픈으로 바꾸는 이유: 같은 종목을 SEC은 `BRK.B`,…, Entity, Any, ValueError, universe가 다루는 사실의 모양. 저장소 행(dict)과 코드가 쓰는 값을 나누는 자리다. dict를 그대로 들고 다니면 컬럼 이름…, 더 깊이 볼 회사. 알림 여부는 여기서 정하지 않는다(notifications의 몫). 관심의 identity는 `cik`다.… (+54 more)
...
Cohesion: 0.09
Nodes (17): NoCandidatesDue, datetime, Protocol, AI 판단 대상을 현재 범용 수집 게이트 구성종목으로 제한한다., 자동 선정에서 지금 다시 판단할 종목이 없다. 오류가 아니라 '이번 회차는 할 일 없음'이다., 한 실행에서 검증된 tracked 전체 집합과 실제 분석 대상을 함께 보존한다., 가장 최근 실행의 마지막 종목 다음부터 순환한다., 명시 종목도 현재 tracked universe 밖이면 fail-closed 한다. (+9 more)

### Community 119 - "BacktestRequest"
Cohesion: 0.14
Nodes (14): BacktestRequest, 네트워크나 DB 없이 재생할 수 있는 완전한 백테스트 입력이다., 시장 이벤트를 순서대로 적용해 동일 입력에 동일 장부를 만든다., WeightBacktestEngine, BacktestComparison, compare_results(), LumiBotValidationEngine, MetricDifference (+6 more)

### Community 120 - "price_risk_profile"
Cohesion: 0.14
...
### Community 121 - "FinRLTrainer"
Cohesion: 0.16
Nodes (6): FinRLTrainer, Any, Path, FinRL의 A2C/DDPG/PPO/SAC/TD3 선택을 최소 SB3 런타임으로 재현한다., _FakeModel, FinRLTrainerTest

### Community 122 - "parse_external_payload"
Cohesion: 0.10
Nodes (19): _clean(), _parse_bracketed_messages(), parse_external_payload(), _parse_markdown_articles(), Any, provider 응답 blob을 기사·게시물 단위 ExternalContent로 쪼갠다. upstream TradingAgents는 항목…, `시각 · @작성자 · 태그` 머리말을 시각·작성자·감성으로 나눈다., 대괄호 머리말이 붙은 게시물을 항목별로 나눈다. 이어지는 줄은 앞 글에 붙인다. (+11 more)
...
Cohesion: 0.10
Nodes (44): _engine_snapshot(), _engine_steps(), _list(), _mapping(), _merge_execution_payload(), Any, 투자 엔진의 입력·모델·신호·위험 심사를 추적하는 읽기 전용 Alpha Lab., Alpha Lab 사실에 실행 원장 배열을 덧붙여 한 번의 추적에서 연결한다. (+36 more)
...
### Community 128 - "Filing13F"
Cohesion: 0.07
Nodes (27): effective_filings(), Filing13F, HoldingsError, portfolio_weights(), Position, Any, ValueError, 13F 보유 신고를 읽는 규칙. ## 정정이 원본을 대체하는 방식이 두 가지다 13F 정정(13F-HR/A)에는 두 종류가 있고, 처리가… (+19 more)

### Community 129 - "What You Must Do When Invoked"
Cohesion: 0.08
Nodes (24): For /graphify add and --watch, For /graphify query, For the commit hook and native CLAUDE.md integration, For --update and --cluster-only, /graphify, Honesty Rules, Interpreter guard for subcommands, Part A - Structural extraction for code files (+16 more)

### Community 130 - "JsonStateStore"
Cohesion: 0.10
Nodes (16): HealthReport, inspect_health(), Any, datetime, 외부 dead-man 점검이 읽을 수 있는 heartbeat·stale 판정., JsonStateStore, Path, 같은 디렉터리의 임시 파일을 원자적으로 교체한다. (+8 more)
...
Cohesion: 0.06
Nodes (25): Filing, FilingError, Any, ValueError, 공시가 존재한다는 사실. 우리가 그것을 어떻게 처리했는지는 여기 없다., canonical `filings` 행. 자식 행의 FK 부모라 먼저 있어야 한다. `Filing.as_row()`와 같은 모양이지만…, FundamentalsRepository, Any (+17 more)
...
### Community 135 - "ml_challengers.py"
Cohesion: 0.15
Nodes (21): default_candidate_dir(), evaluate_candidates(), _icir(), _load_champion(), main(), Any, datetime, Path (+13 more)
...
Cohesion: 0.05
Nodes (19): FilingRef, 원천 수집기가 넘기는 SEC 공시 참조. ``Filing``은 v1 저장 계약이고, 이 타입은 아직 원문을 읽기 전의 최소 식별자다. 둘을…, BackfillScopeTest, BulkFilingContractTest, _CompanyRepository, _CompanySource, FilingSchemaContractTest, date (+11 more)
...
Cohesion: 0.06
Nodes (48): parse_8k_earnings(), 8-K 실적 발표에서 저장 가능한 요약값을 조립한다., 내재화한 표·본문 파서로 8-K 실적값을 한 번에 추출한다., clean_html_to_markdown(), _document_score(), extract_guidance_text(), extract_text(), _item_value() (+40 more)
...
Cohesion: 0.08
Nodes (37): _beat_color(), _expectation_view(), _inconsistent_7d(), 상회=초록·하회=빨강. 기준이 어긋난 값은 색으로 단정하지 않는다., 상·하향 건수를 해석 문장으로. 서프라이즈를 읽는 전제가 된다., 7일 값이 30일 값을 넘는가 — 출처가 두 창을 따로 갱신해 생기는 어긋남. 7일은 30일에 포함되므로 논리적으로 30일이 크거나 같아야…, consensus.build() 결과를 템플릿이 그대로 쓸 수 있는 표기로 바꾼다., revisions_text() (+29 more)
...
Cohesion: 0.13
Nodes (13): DossierBuilder, bundle 하나와 선택적 밸류에이션 관측값으로 서류철을 만든다., BuilderTest, _bundle(), BuilderWiringTest, 장기 조회가 없어도 서류철은 만들어진다 — 섹션만 얕아진다., price_statistics의 실제 키와 어긋나면 값이 조용히 사라진다., _item() (+5 more)
...
Cohesion: 0.10
Nodes (16): range, control_command(), ControlCenter, dashboard_command(), dashboard_url(), Any, Path, ATLAS 로컬 제어센터. 대시보드는 관측 전용으로 유지하고, 하네스 시작·정지처럼 로컬 프로세스를 변경하는 작업만 이 창에서 명시적으로… (+8 more)

### Community 150 - "src/investment_agent/research/evaluation/__init__.py"
Cohesion: 0.13
Nodes (14): ChallengerComparison, ChallengerPolicy, compare_challenger(), Any, Champion과 Challenger의 비교 결과를 수동 승격 입력으로 만든다., OOS·shadow·paper 증거를 평가하되 자동 운영 승격은 항상 금지한다., 연구 metric과 OOS stability 평가., evaluate_predictions() (+6 more)

### Community 151 - "collection.py"
Cohesion: 0.08
Nodes (34): _missing_sec_get_json(), Any, SEC·S&P500·Toss 수집 흐름을 조율한다., 수집 게이트를 현재 멤버 집합에 맞춘다. 고친 종목 수를 돌려준다., 현재 S&P 집합을 비교하고 실제 변경 또는 월간 감사 때만 저장한다. `is_tracked`는 범용 gate이므로, 과거 멤버 전체를…, SEC 거래소 master와 시세 기준 ETF를 동기화하고 새 CIK의 entity 이름만 seed한다. ETF는 매번 함께 넘긴다. 빠지면…, CIK별 SEC submissions metadata를 증분 보강한다., reconcile_membership() (+26 more)

### Community 152 - "institutional/persistence.py"
Cohesion: 0.12
Nodes (28): 재시도 가능한 CUSIP/CINS만 다시 조회하고 universe로 후보를 검증한다., 이미 적재된 전체 13F의 미완료 CUSIP/CINS 매핑을 별도로 복구한다. SEC 원천 적재와 API rate-limit 장애를 분리하기…, refresh_mappings(), _resolve_tickers(), cache_mappings(), _clear_unconfirmed(), _db(), delete_filings_before() (+20 more)
...
### Community 154 - "WatchlistTest"
Cohesion: 0.17
Nodes (4): 관심은 발행사(CIK) 단위다. 표시 ticker는 읽기 경계에서 대표 종목으로 붙인다., GOOG/GOOGL이 둘 다 tracked여도 관심 기업은 하나고, 대표는 매번 같아야 한다., 보여줄 ticker가 없으면 빈 껍데기를 만들지 않는다., WatchlistTest
...
### Community 156 - "directory.py"
Cohesion: 0.16
Nodes (13): build_directory(), GuildChannel, GuildDirectory, _normalize(), Any, 길드의 채널을 **이름으로** 찾는다. 채널 ID를 환경변수로 늘리지 않기 위해서다. ## 왜 이름으로 찾나 거장 채널은 한 사람당 하나였고,…, Discord는 채널 이름을 소문자로 정규화한다. 비교도 같은 기준으로 한다., API 응답을 이름 색인으로 접는다. 순수 함수라 네트워크 없이 시험할 수 있다. (+5 more)

### Community 157 - "SignalBatch"
Cohesion: 0.22
...
### Community 159 - "_run"
Cohesion: 0.15
Nodes (12): AccountingTest, ChainTest, EmbedTest, 체인 감시·연속 일수·심각도 색 — 전부 '조용히 틀리는' 자리다., cron이 없는 워크플로는 상류 성공 말고는 '돌았어야 함'을 알 길이 없다., 상류가 방금 끝났으면 하류는 아직 큐에 있을 수 있다., 상류가 실패했으면 하류가 안 도는 게 맞다 — 그걸 하류 탓으로 세면 안 된다., 인프라 타임아웃 이틀 뒤 코드가 깨진 것을 '3일째 실패'라 부르면 안 된다. (+4 more)
...
### Community 164 - "FeatureDataset"
Cohesion: 0.04
Nodes (46): PPO의 초기 범위를 allocation·timing으로 제한한다., RewardConfig, action_to_weights(), FeatureDataset, make_gym_environment(), _project_to_constraints(), Any, ndarray (+38 more)

### Community 165 - "DeliverEditTest"
Cohesion: 0.17
Nodes (4): _config(), DeliverEditTest, 원장 기반 전송(`deliver`)과 정정(`edit`)이 Discord에 어떤 요청을 보내는지 굳힌다., _Response

### Community 166 - "request_toss_approval.py"
Cohesion: 0.03
Nodes (94): build_approval_card(), button_components(), Any, 토스 비실행 주문표를 Discord 건별 승인 카드로 바꾼다., 자유문장 대신 서명된 승인/거절 버튼 두 개만 만든다., Discord message create API에 바로 전달할 안전한 payload를 만든다., _ticket_lines(), DiscordApprovalClient (+86 more)

### Community 167 - "market/persistence.py"
Cohesion: 0.08
Nodes (50): _backfill_prices(), 계획을 한 번 고정한 뒤 받아 archive → 기업행위 병합 → 가격 저장 순으로 반영한다., main(), merge_corporate_actions(), 분리 저장한 배당·분할 이벤트를 가격 행에 읽기 전용으로 결합한다., 가격 행의 grain을 바꾸지 않고 같은 거래일의 이벤트만 붙인다., close_history_as_of(), closes_on_date() (+42 more)

### Community 168 - "FakeDatabase"
Cohesion: 0.02
Nodes (71): 13F 유스케이스 계층. 수집·적재·정리 흐름을 조립한다., 기관 보유 신고(13F). 정정에는 **전체 재신고**와 **누락 추가** 두 종류가 있고 처리가 정반대다. 그 구분은…, MembershipSnapshot, 그날 지수에 무엇이 있었는가. 이것이 없으면 backtest가 생존 편향에 걸린다 — 지금 살아남은 종목만 과거에 넣게 되기 때문이다. 그래서…, ActiveManagerQueryTest, RadarCodeContractTest, 7인 레이더 코드 계약 + manager_groups 스냅샷 + active 수집 기준 테스트. 외부 의존성 없음(DB/네트워크 미접근).…, manager_cik/name/fund_name/is_active의 SSOT는 코드 설정이다 — DB는 전혀 관여하지 않으므로… (+63 more)

### Community 169 - "classify_session"
Cohesion: 0.22
Nodes (13): classify_session(), collect_window(), is_placeholder_time(), datetime, time, 발표 예정 시각을 거래 세션 구간(BMO/AMC)으로 분류한다. 발표 '날짜'만으로는 언제 수집을 걸어야 하는지 알 수 없다. 같은 날짜라도…, ET 기준 발표 시각을 세션으로 분류한다. tz 정보가 없는 값은 ET로 간주한다 — yfinance의 earnings_dates 인덱스는…, 야후가 '시각 미정'에 쓰는 자리표시 시각인지. (+5 more)

### Community 170 - "MacroRepository"
Cohesion: 0.05
Nodes (30): Any, v1 시장 거시지표의 선언형 원천 카탈로그. 카탈로그는 DB seed와 같은 계약을 코드에서 재현한다. 수집 시에는 DB에 실제로 seed된…, _row(), Observation, 관측 한 버전. 같은 `(series_id, ref_period)`에 여러 개가 있을 수 있다., MacroRepository, Any, Database (+22 more)

### Community 171 - "ml_rl_lab.py"
Cohesion: 0.11
Nodes (17): Figure, 선택된 워크스페이스만 실행되는 Streamlit 페이지 모음., _load_real_active_policy(), Any, AI·강화학습(ML/RL) 자율진화 관제 랩 대시보드 페이지. 하네스 7대 전자동 잡의 실제 실행 상태, PPO 강화학습 챔피언 정책…, 실제 강화학습 승격 정책 메타데이터 로드 (없으면 None)., DSR 과적합 검정 반원형 게이지 차트 생성., render_dsr_gauge() (+9 more)

### Community 172 - "us_market_today"
Cohesion: 0.07
Nodes (21): Clock, completed_us_daily_bar_cutoff(), day_window(), FixedClock, date, datetime, Protocol, 뉴욕 시장이 관측하는 날짜를 반환한다. (+13 more)

### Community 173 - "build_features"
Cohesion: 0.10
Nodes (10): build_features(), default_workers(), datetime, 종목별 EvidenceBundle을 고정 스키마 FeatureSnapshot으로 바꿔 저장한다., 환경변수가 없거나 잘못되면 기본값. 상한을 넘기면 상한으로 자른다., 그 실행 시각까지의 최신 PIT 밸류에이션 관측값을 종목별로 고른다. 원장이 아직 비어 있거나 조회가 실패해도 feature 적재 자체는 막지…, _valuations_by_ticker(), BuildFeaturesEntryTest (+2 more)

### Community 174 - "ResearchDataset"
Cohesion: 0.07
Nodes (31): Hashable, ndarray, 모델 입력으로 사용할 때만 label을 결합한 immutable matrix., ResearchDataset, cross_sectional_alpha_metrics(), CrossSectionalAlphaScore, Any, ndarray (+23 more)

### Community 175 - "ContextBuilder"
Cohesion: 0.08
Nodes (10): ContextBuilder, _CountingRepository, IncrementalSaveTest, feature 적재: 종목과 무관한 조회는 한 번만, 계산은 쌓이는 대로 저장한다., SharedContextTest, ParallelFeatureBuildTest, ContextPointInTimeTest, historical replay에서 point-in-time 이력이 없는 데이터를 차단한다. (+2 more)
...
Cohesion: 0.14
Nodes (9): BuildTrainingSamplesTest, _features(), datetime, 비용 반영 학습 표본이 gross label을 그대로 베끼지 않는지 고정한다., 과거 편출 종목 BBB를 조회 범위에서 빼면 학습 표본이 조용히 사라진다., +0.1% 판단은 비용 후 손실이다. 그 개수가 보고돼야 한다., 최소 수수료가 있으면 비율이 주문 금액에 따라 달라져 계산할 수 없다., 적재 첫 며칠은 horizon이 안 지나 label이 0건이다. 여기서 실패시키면 5거래일 내내 job이 실패로 뜨고, 그 사이 정상 적재된… (+1 more)
...
Cohesion: 0.10
Nodes (26): PositionKey, SEC Information Table 원본 행의 분석·감사 필수 필드., RawPosition, identifier_type(), 13F의 9자리 식별자를 OpenFIGI 조회 타입으로 분류한다. CINS는 국가 문자로 시작하는 9자리 CUSIP 확장이다. 이 분류는…, _aggregate(), compare(), errored() (+18 more)
...
Cohesion: 0.13
Nodes (15): ExecutionRepository, main(), Paper intent 생성 및 승인 요청 진입점., 부분 분석이나 실제 계좌 기준이 없는 목표 비중을 주문으로 승격하지 않는다., validate_paper_scope(), create_execution_intent(), main(), datetime (+7 more)

### Community 184 - "validated_weights"
Cohesion: 0.11
Nodes (13): IntentError, Any, datetime, ValueError, 지금 이 의도로 주문을 만들어도 되는가. 아니면 예외. `required_mode`를 인자로 받는 이유: 부르는 쪽이 **자기가 어느 경로인지…, 실행 의도가 계약을 어겼다. **주문을 만들지 않는다.**, 비중을 검증하고 현금을 포함한 정렬 사본을 돌려준다. **현금을 포함한 합이 1이어야 한다.** 이것이 이 함수의 핵심이다 — 없으면 "10%…, validated_weights() (+5 more)

### Community 185 - "layer.py"
Cohesion: 0.19
Nodes (20): _close_returns(), FeatureBundle, _fundamental(), _gurus(), _macro(), _momentum(), _number(), Any (+12 more)

### Community 186 - "company_financials.py"
Cohesion: 0.07
Nodes (35): _ensure_filings(), filing_accessions(), _filing_state_row(), gating_universe(), _issue_key(), last_filed_map(), mark_empty_filing_targets(), mark_processed_filing_targets() (+27 more)
...
### Community 190 - "FactorScore"
Cohesion: 0.12
Nodes (11): FactorScore, _by_symbol(), ExpectedReturnSignalsTest, ALPHA: IC×σ×z 기대수익, 논지의 거부권·소폭 조정·검증 전 신규 금지. 사고팔기 단어는 입력이 아니다., 행동 단어와 수치가 엇갈리면 수치를 따른다., _scores(), ThesisStateTest, _view() (+3 more)

### Community 191 - "src/investment_agent/data/fundamentals/application/__init__.py"
Cohesion: 0.04
Nodes (46): SimpleNamespace, 과거 8-K 실적 속보에 EPS 예상치 재구성값을 채운다., detect_earnings_events(), detect_earnings_events_for_ticker(), determine_fiscal_period(), _parse_release_html(), Any, date (+38 more)

### Community 192 - "TossAuthError"
Cohesion: 0.07
Nodes (37): _access_token(), _fetch_chunk(), fetch_korean_names(), toss.py — 토스증권 Open API에서 미국 종목의 한글명을 보강한다. OAuth2 client_credentials로 access…, 공용 token을 반환하고 실패는 미시도 상태로 호출자에게 전달한다., 우리 표기(BRK-B) → 토스 표기(BRK.B)., 종목코드 목록 → (찾은 {ticker: 한글명}, 실제로 조회한 ticker 목록). 두 번째 값(attempted)은 '실제로 토스에…, _to_toss_symbol() (+29 more)
...
### Community 196 - "_row"
Cohesion: 0.16
Nodes (9): JudgeTest, 채널 도착 확인 — '워크플로 초록인데 카드 없음'을 잡는 유일한 장치다. 여기가 틀리면 조용히 틀린다: 거짓 경보를 내면 며칠 만에 아무도…, 포럼의 글은 메시지가 아니라 스레드라 셀 수 없다 — 없는 수를 지어내지 않는다., 워크플로가 전부 초록이어도 채널이 비었으면 '이상 없음'이라고 하면 안 된다., 발표·공시가 있어야 우는 채널을 조용하다고 경보하면 매일 거짓 경보가 뜬다., 어제 만든 채널을 '오래 조용하다'고 울면 첫 카드까지 며칠을 거짓으로 운다., 시크릿을 빠뜨린 날 감시 대상에서 조용히 빠지면 아무도 모른다., RenderTest (+1 more)
...
Cohesion: 0.14
Nodes (27): main(), AlfredDataError, _failure_reason(), fetch_batch(), fetch_first_prints(), fetch_revisions(), _http_error_message(), _is_empty_vintage_window() (+19 more)

### Community 202 - "OpenAICompatibleClient"
Cohesion: 0.09
Nodes (17): LLMClient, OpenAICompatibleClient, _parse_json_object(), Any, Protocol, OpenAI 호환 로컬·원격 LLM 호출 어댑터., 모델이 실제로 받는 모양으로 요청 본문을 만든다., 이 모델에 temperature를 실어 보내도 되는가. (+9 more)
...
Cohesion: 0.14
Nodes (21): _authoritative_filings(), build_snapshot(), _change_rows(), _combined_positions(), _consensus_rows(), _coverage_status(), _filing_order(), _instrument() (+13 more)

### Community 207 - "application/etl.py"
Cohesion: 0.10
Nodes (26): AllGuruManagersFailedError, GuruConfigurationError, GuruEtlError, GuruProviderError, main(), _make_filing_error_sink(), _make_shadow_sink(), Any (+18 more)
...
### Community 211 - "ActualProviderTest"
Cohesion: 0.06
Nodes (11): ActualProviderTest, _fred_setting(), ECON actual provider의 unit/validation/fail-closed 계약., ActiveSeedContractTest, ActualsFailClosedTest, MarketBreadthFailClosedTest, 활성 provider의 오프라인 fail-closed 계약 테스트., 운영 seed의 provider 설정이 코드 registry와 어긋나지 않는지 검사한다. (+3 more)

### Community 212 - "Event"
Cohesion: 0.13
Nodes (13): Popen, CommandExecutionError, Path, RuntimeError, shell 없이 고정된 Python entry만 실행하는 하네스 command 경계., 종료 요청은 자식에게 먼저 전달하고 짧게 기다린 뒤 강제 종료한다., 인자나 자격증명 없이 command의 실패 유형만 상위 상태머신에 알린다., argv 배열만 사용하고 stdout/stderr를 credential 보관소로 만들지 않는다. (+5 more)
...
### Community 219 - "process_company_facts"
Cohesion: 0.10
Nodes (17): process_company_facts(), Any, 표준 fact를 기업 전체 재무로 만들고 저장한다., filing_row(), 어떤 모양으로 오든 canonical `filings` 행 하나로 바꾼다. `Filing`(저장 계약)과 `FilingRef`(원문 읽기 전…, build_company_financials(), CompanyFinancialBatch, 표준 공시 fact에서 기업 전체 재무 결과를 만든다. (+9 more)
...
Cohesion: 0.06
Nodes (35): ALPHA — 기대수익과 논지, Dataset split과 재현성, DeterministicRiskGate, EvidenceBundle 도메인 구성, factor IC 연구, factor 기반 후보 선정, Feature Layer, LumiBot 외부 검증 (+27 more)

### Community 223 - "process_filing.py"
Cohesion: 0.07
Nodes (45): derive_segment_rows(), persist_segment_filings(), process_segment_cik(), date, 공시 한 건의 기업 전체·세그먼트 결과를 처리한다., 세그먼트 공시 처리 상태를 저장소 행으로 만든다., CIK 한 건의 새 공시 문서를 파싱해 저장 전 결과를 만든다., 저장된 연도 지표와 이번 YTD 관측값으로 discrete 분기를 계산한다. (+37 more)

### Community 224 - "_canonical_filing_focus"
Cohesion: 0.15
Nodes (13): _canonical_filing_focus(), filing_focus(), _filing_focus_plan(), _quarter_from_annual_distance(), 인접 10-K 기간말과의 거리로 10-Q의 분기 번호를 복원한다., SEC unit의 오염된 ``fy``/``fp`` 대신 filing 기간으로 회계키를 만든다. CompanyFacts는 일부 등록인에서 과거…, 중복 회계력 전환까지 접은 canonical focus와 대체 accession을 계산한다., accession별 canonical 회계연도·기간을 한 번에 계산한다. (+5 more)

### Community 225 - "trading/repository.py"
Cohesion: 0.11
Nodes (14): trading 스키마를 읽고 쓰는 유일한 자리. ## 한 회차의 흐름이 표로 남는다 ``` decision_runs →…, ContractViolation, Any, ValueError, 판단·실행 전체가 공유하는 어휘. ## 세 축은 서로 독립이다 한 낱말로 뭉뚱그리면 조용히 틀린다. 실제로 `mode`라는 이름 하나에 세…, 한 판단 회차가 서 있는 자리. 기록되는 모든 행이 이 셋을 갖는다., 실제 돈이 움직이는가. 안전 점검이 보는 유일한 값이다., 기본값. 판단만 하고 주문은 종이로 간다. (+6 more)
...
### Community 230 - "PortfolioRiskPolicy"
Cohesion: 0.12
Nodes (10): DeterministicRiskGate, PortfolioRiskPolicy, 코드 리뷰 없이 모델이 변경할 수 없는 long-only 위험 한도., 꼬리 손실이 변동성 상한의 정규분포보다 나쁘지 않을 것. 변동성 한도는 좌우 대칭으로 위험을 본다. 같은 변동성이라도 급락이 몰려 있는…, 어떤 시나리오에서도 "베타 상한 포트폴리오가 시장 -10%에서 잃는 만큼"보다 더 잃지 않는다. 베타 한도는 시장 하나만 본다.…, _exits_proposal(), MandatoryTurnoverTest, proposal() (+2 more)

### Community 233 - "ResearchStore"
Cohesion: 0.06
Nodes (35): load_prev_alloc(), load_recent_allocations(), Research 로컬 전략 결과를 알림용 모양으로 읽는 저장소 경계. 보낼지 말지는 알림 원장이 판단한다., 최근 적용월의 전략 배분을 오래된 순서로 읽는다., _store(), 저장 기술별 연결 경계. 각 파일이 하나의 DB만 안다. 이 패키지는 재수출하지 않는다. `platform.db`만 적으면 세 저장소 중…, _decode_allocation(), _decode_json() (+27 more)

### Community 234 - "NotificationLedger"
Cohesion: 0.15
Nodes (8): _as_datetime(), NotificationLedger, Any, Protocol, 알림 원장의 계약. 저장소는 Supabase `notifications` 스키마 하나다(`notifications/db.py`). 원장이…, 이 실행이 맡은 알림 하나. action이 무엇을 할지 말한다., Reservation, _Row

### Community 235 - "build_events"
Cohesion: 0.15
Nodes (10): build_events(), EventRepository, _normalized(), Any, Protocol, 행 하나가 계약을 어겨도 그 행만 버린다 — 배치 전체를 잃지 않는다., cutoff 이전 원문만 읽어 사건을 만들고 종목별 feature snapshot을 남긴다., BuildEventsTest (+2 more)
...
### Community 237 - "select_timed_targets"
Cohesion: 0.15
...
Cohesion: 0.06
Nodes (28): _args(), main(), Namespace, aggregate_evaluations(), _covered_days(), EvaluationSummary, _incident_count(), ManualPromotionGate (+20 more)

### Community 239 - "test_read_path_performance.py"
Cohesion: 0.11
Nodes (8): _FakeMarketRepository, LatestFeaturesBatchTest, PriceWindowTest, date, 읽기 경로 최적화가 결과를 바꾸지 않는지: 가격 창 조회·종목 ID 기억·저장소 기억·기술지표 일괄·병렬 feature 적재., RepositoryMemoTest, SecurityIdCacheTest, _weekday_bars()
...
Cohesion: 0.07
Nodes (28): event_key(), Any, date, 경제발표의 자연키를 화면·알림·적재에서 동일하게 읽고 만든다., row_event_key(), build(), _escape(), _event_lines() (+20 more)
...
Cohesion: 0.13
Nodes (15): _atomic_json(), load_backtest_input(), main(), _mapping(), Any, Path, 완전한 오프라인 JSON 입력을 결정론적 백테스트 artifact로 변환한다., BacktestConfig (+7 more)

### Community 246 - "refresh_earnings_season.py"
Cohesion: 0.15
...
### Community 251 - "TossOrderApiTest"
Cohesion: 0.20
Nodes (8): command(), controls(), FakeResponse, permit(), remote_order(), risk_state(), TossOrderApiTest, TossOrderCommandTest

### Community 252 - "fusion.py"
Cohesion: 0.19
Nodes (15): AnalystSignal, Market/Fundamental/Macro/Event desk가 공통으로 반환하는 신호., _clamp(), _desk_returns(), _finite(), fuse_signals(), FusionResult, NumericPrediction (+7 more)
...
### Community 260 - "._at"
Cohesion: 0.15
Nodes (8): 시간 메타데이터가 없다고 발표를 놓치는 쪽이 더 나쁘다., 야후는 시각 미공지 발표에 15:00 ET를 넣는다. 그대로 믿으면 창이 어긋난다., 실측: AMAT는 과거 24회가 16:00인데 다음 예정만 15:00이었다., 확정 시각이 있으면 과거 습관이 달라도 그대로 믿는다., 이력이 없으면 장중으로 남지만, 창은 하루 전체다 — 놓치는 쪽이 더 나쁘다., 과거 행에 섞인 자리표시는 최빈값 계산에서 빠진다., SessionTargetSelection, YahooPlaceholderTime

### Community 261 - "update_performance"
Cohesion: 0.06
Nodes (30): main(), latest_performance(), 성과 owner의 지속 보고서를 읽는 알림 계약., account_fills(), nav_returns(), number(), 체결 FIFO와 현금흐름 구간 수익률. 원가·비용·평가 증거가 없으면 미확인이다., 금액과 수량의 NaN·무한·boolean을 거부한다. (+22 more)
...
### Community 263 - "segment_concepts.py"
Cohesion: 0.13
Nodes (28): canonical_axis(), canonical_member(), canonical_segment_dimensions(), display_member_name(), is_aggregate_member(), local_name(), 세그먼트 XBRL fact의 축과 멤버를 표준 차원으로 분류한다., 원본 차원을 버리지 않고 축·멤버 local name만 정규화한다. (+20 more)

### Community 264 - "build_decision_experiences.py"
Cohesion: 0.07
Nodes (28): build_experience(), main(), datetime, 원본 판단의 당시 신호와 독립적인 비용 반영 가상 성과를 연결한다., 다음 날 이후 종가 진입과 완전히 지난 거래일만 사용한다. 보유 비중을 추측하지 않는 단위 노출 실험이다. open/increase는 1 단위…, 이미 관측한 경험은 덮어쓰지 않고 신규 성숙 판단만 추가한다., run(), main() (+20 more)
...
Cohesion: 0.22
Nodes (17): _configured_path(), evidence_cache_path(), intelligence_database_path(), intelligence_parquet_root(), legacy_candidates(), local_artifact_root(), local_data_root(), market_change_manifest_path() (+9 more)

### Community 267 - "_db"
Cohesion: 0.07
Nodes (15): _db(), _FakeBuilder, _FakeClient, _FakeTable, Any, Database, 조용히 잘리거나 빠지는 읽기를 막는지 본다., 정렬 없는 range 읽기는 페이지 경계로 행이 빠질 수 있다. 조용히 두지 않는다. (+7 more)
...
### Community 270 - "institutional/domain/__init__.py"
Cohesion: 0.10
Nodes (8): skipUnless, 13F 도메인 규칙. 외부 호출도 저장소도 모른다., EdgartoolsWrapperTest, edgartools shadow 파서 wrapper와 운영 파서의 결과 일치(parity) 테스트. edgartools가 설치되어 있지 않으면…, edgartools 미설치/지연 import 경로. 설치 여부와 무관하게 동작해야 한다., edgartools 출력 컬럼을 운영 모델로 정규화하는 순수 헬퍼. 의존성 불필요., WrapperNormalizerTest, WrapperUnavailableTest
...
Cohesion: 0.16
Nodes (8): macro: 매크로 시장 지표 알림 묶음 (core·watch). 데이터 조회와 알림 조립은 이 패키지의…, _marker_tier(), 매크로 알림 — 경보는 상태가 바뀔 때만, 오늘의 시장은 내용이 바뀔 때만 사람에게 닿는다., 관측마다 등급 표식을 심어 둔 지표 한 줄. 등급 판정은 표식을 그대로 돌려준다., 관측일 하나로 원장을 치면 늦게 들어온 다른 지표의 새 경보가 버려진다., _series(), WatchPublishTest, WatchStateTest

### Community 275 - "map_fiscal_periods.py"
Cohesion: 0.12
Nodes (24): _add_months(), _annual_period(), _as_date(), _as_datetime(), _calendar_by_ticker(), _coherent_range(), _latest_reported_quarters(), _nearest_period() (+16 more)

### Community 276 - "object"
Cohesion: 0.24
Nodes (9): object, 풀에서 남은 후보를 하나씩 예약·시도해 이 종목의 분석을 완주한다. 한 후보가 실패하면(예약 자체가 없거나 호출 도중 429/404 등) 다음…, _select_and_run(), _candidate(), NothingDueTest, Path, analysis의 모델 풀 재시도 배선 — 실제 TradingAgents/네트워크는 주입으로 뗀다., 예약은 시도 즉시 소진되므로 실패한 후보를 같은 종목 안에서 두 번 고르지 않는다. (+1 more)
...
### Community 279 - "portfolio/contracts.py"
Cohesion: 0.08
Nodes (32): compute_ml_fusion(), fuse_proposals(), fusion_enabled(), latest_cross_section(), load_active_model(), MlFusionOutcome, ModelPrediction, Any (+24 more)
...
### Community 282 - "history.py"
Cohesion: 0.11
Nodes (22): _annual_points(), AnnualPoint, _bars_oldest_first(), _cagr(), fundamental_trend(), _gap_profile(), _number(), _percentile() (+14 more)
...
### Community 285 - "postgres.py"
Cohesion: 0.07
Nodes (23): Client, delete_history_before(), 보존 하한 이전의 지표와 공시 상태를 외래키 순서대로 제거한다. ``segment_metrics``가 ``filings``를 참조하므로 자식…, anon_client(), chunk_filter_values(), chunk_values(), _LazyServiceClient, Any (+15 more)
...
### Community 289 - "json_value"
Cohesion: 0.12
Nodes (12): json_value(), JSON에 실을 수 있는 모양으로 바꾼다. 손실이 생기는 변환은 하지 않는다., CorporateAction, Any, 장 시작 전에 포지션에 적용할 split 또는 현금배당이다., AttributionReport, build_attribution_report(), _finite() (+4 more)
...
Cohesion: 0.21
Nodes (16): _activity_rows(), build_quarterly(), _co_held(), _conflicts(), _latest_filings(), _latest_period(), _manager_labels(), _radar_status() (+8 more)

### Community 294 - "stress.py"
Cohesion: 0.14
...
### Community 295 - "earnings_report/consensus.py"
Cohesion: 0.18
...
### Community 298 - "storable_share_rows"
Cohesion: 0.26
...
### Community 305 - "harness/runtime.py"
Cohesion: 0.06
Nodes (33): main(), Discord 승인 Gateway listener를 별도 lock으로 감싸는 로컬 서비스 entry., DuplicateProcessError, ProcessFileLock, Path, RuntimeError, 한 장비에서 하네스 프로세스가 하나만 실행되게 하는 OS file lock., 이미 같은 state 디렉터리를 소유한 프로세스가 있는 경우다. (+25 more)
...
Cohesion: 0.11
Nodes (13): 이번 주 실적 발표 예정 카드. 발표 예정일 스냅샷을 읽어 관심종목의 다음 발표를 미리 알린다. 예정일은 자주 바뀌고 틀리기도 하므로 카드는…, PendingStateTest, memory_context(), datetime, 모든 topic의 baseline을 과거로 둔 메모리 원장과 가짜 채널., CalendarTest, _flash(), FlashTest (+5 more)
...
Cohesion: 0.07
Nodes (47): analyze_allocation_change(), Allocation change analysis for strategy notifications., _as_card(), _as_row(), build_card(), build_summary(), _generic_layout(), _layout_for() (+39 more)

### Community 310 - "discord_admin/client.py"
Cohesion: 0.04
Nodes (109): add_member_role(), admin_user_id(), bot_user_id(), create_channel(), create_role(), create_webhook(), delete_channel(), edit_channel() (+101 more)
...
Cohesion: 0.16
Nodes (12): ComputeRlTargetWeightsTest, LiveMembershipTest, _model(), datetime, Exception, Path, 승격된 RL 정책을 판단 시점에 불러 challenger 목표비중을 만드는 배선. 못박는 것: 정책이 없거나 feature가 비어도 예외 없이…, 하루보다 오래된 snapshot은 마스킹되어 비중이 현금으로 간다. (+4 more)

### Community 315 - "_Query"
Cohesion: 0.14
Nodes (3): _Query, _Response, _Schema

### Community 316 - "adopt_ml_model.py"
Cohesion: 0.23
Nodes (9): AdoptionCheck, check_adoptable(), main(), _parse_args(), Any, Namespace, 학습된 ML artifact를 판단 경로가 읽는 채택 모델로 올린다. `ml_serving`은 `active_ml_model.json`이…, AdoptMlModelTest (+1 more)
...
Cohesion: 0.08
Nodes (12): CommandResult, context(), FakeApprovalRepository, FakeDecisionRepository, FakeRunner, FakeSystemStore, InvestmentAdaptersTest, 거절·만료도 물은 것이다. 같은 System 목표로 다시 승인을 묻지 않는다. (+4 more)
...
### Community 320 - "PPOPolicy"
Cohesion: 0.15
Nodes (4): PPOPolicy, ContinuousRetrainTest, 원장 세 갈래를 그대로 흉내내는 fake. 네트워크를 쓰지 않는다., _Repository

### Community 321 - "archive_daily_rows"
Cohesion: 0.25
Nodes (13): archive_daily_rows(), archive_root(), _frame(), MarketArchiveError, Any, DataFrame, Path, RuntimeError (+5 more)

### Community 322 - "keys.py"
Cohesion: 0.11
Nodes (16): case_key(), context_hash(), is_reproducible(), Any, datetime, 판단 하나를 가리키는 키와, 그 판단을 재현할 수 있는지 보는 지문. ## case_key는 계산되는 값이다 UUID를 쓰면 같은 판단을 두…, 마이크로초를 버린다. 재실행이 같은 키로 모이게 하는 유일한 이유다., 판단 하나의 키. 같은 질문이면 항상 같은 문자열. 사람이 읽을 수 있게 앞부분을 그대로 두고 뒤에 지문을 붙인다 — 로그에서 어느 종목·어느… (+8 more)
...
### Community 324 - "refresh_market_state.py"
Cohesion: 0.09
Nodes (17): SeriesValidator, SourceFetcher, MacroRefreshResult, Any, date, datetime, v1 macro 원천 결과를 revision-safe observation 원장에 적재한다., source별 결과를 독립 수집해 원천 수집 시각으로만 버전을 남긴다. (+9 more)
...
### Community 326 - "MarketRetentionTest"
Cohesion: 0.18
Nodes (3): MarketRetentionTest, v1 market은 append-only 관측 원장이다 — 보존 정리는 아무것도 지우지 않는다. 배치 삭제가 돌아오면 그것이 8초…, 뒷정리 실패가 그날의 수집을 되돌리면 안 된다. 2026-08-27·28에 실제로 그랬다 — 가격을 전부 저장한 뒤 보존 정리가…

### Community 327 - "harness_adapters.py"
Cohesion: 0.11
Nodes (19): ApprovalRepositoryPort, _clock(), DecisionRepositoryPort, follow_system_target(), _positive_float(), _positive_int(), Any, datetime (+11 more)

### Community 328 - "monitoring/__init__.py"
Cohesion: 0.12
Nodes (6): 예정된 것이 실제로 돌았고 실제로 도착했는지 본다. 여기 있는 것은 파이프라인을 돌리지 않는다 — 돈 결과를 읽고 사람에게 말한다. cron이…, CronMatchTest, FiresBetweenTest, cron 판정 — 여기가 틀리면 '미실행'을 거꾸로 보고한다., cron 요일은 일=0이다. 파이썬 weekday()(월=0)를 그대로 쓰면 하루씩 밀린다., market_daily는 전날 23:30 UTC다 — 구간을 24시간으로 잡아야 잡힌다.
...
### Community 334 - "InvestmentDossier"
Cohesion: 0.12
Nodes (14): DossierQuality, InvestmentDossier, Any, 한 종목·한 시점의 근거 묶음. LLM 요약과 ML feature가 공유하는 원본이다., 서류철을 믿을 수 있는 정도를 숫자와 문장으로 함께 남긴다., LLM 판단에 쓸 수 있는 섹션 비율. 0으로 나누지 않는다., _format_number(), Any (+6 more)
...
### Community 338 - "test_watchlist_toss.py"
Cohesion: 0.12
...
### Community 343 - "monitoring/discord.py"
Cohesion: 0.23
...
Cohesion: 0.18
Nodes (6): CollectNewsTest, _payload(), 조회해서 받은 것은 '화제'가 아니다 — 등급을 남긴다., 저장한 뒤 다음 정리에서 지우면 그 사이 화면에 잠깐 나타난다., 파싱 실패를 중복으로 세면 '조용한 0건'을 못 알아본다., cap 소진은 provider 오류가 아니다 — 남은 종목을 계속 돌면 예약만 반복 소모한다.
...
Cohesion: 0.13
Nodes (17): normalize_tz(), Logger, Series, 날짜의 timezone 정보를 떼어 형식 통일(yfinance 등 tz 표기 제각각 대응)., 지표를 하나씩 fetch. 하나 실패해도 멈추지 않고 성공값+실패목록을 함께 반환. budget_sec를 넘기면 그 시점부터 남은 지표는…, safe_fetch(), fetch_batch(), _fetch_one() (+9 more)
...
### Community 352 - "freshness_for"
Cohesion: 0.13
Nodes (11): freshness_for(), kst_today(), date, 매크로 관측값의 기준일과 지연 상태를 일관되게 계산한다., 실행 위치와 관계없이 한국 시간의 오늘을 반환한다., 관측 기준일의 나이와 UI/알림용 freshness 상태를 반환한다. 일간 시세와 감시 지표의 지연 상태에 사용한다., MacroEconBoundaryTest, MacroFreshnessTest (+3 more)
...
### Community 354 - "notifications/macro.py"
Cohesion: 0.16
Nodes (13): MacroNotificationStore, Any, Database, 매크로 알림용 v1 조회 저장소. 보낼지 말지는 알림 원장이 판단한다., 원천 관측을 카드가 소비하는 최신값+파생지표 행으로 접는다. `history`에는 최근 관측마다 경보 판정에 필요한 값(현재·직전 값과…, _rows_from_window(), compute_metrics_series(), DataFrame (+5 more)
...
### Community 357 - "worker.py"
Cohesion: 0.19
Nodes (10): PositionMismatch, classify_remote_order(), datetime, 토스 주문 상태를 immutable snapshot과 로컬 상태로 재동기화한다., broker_order_id가 있는 주문만 자동 결합하고 나머지는 운영자 확인으로 남긴다., 완전히 체결된 주문의 비용을 승인 기준가 대비로 한 번 기록한다. 이 기록이 없으면 체결 비용은 브로커 스냅샷 안에만 흩어져 있고,…, 새 enum은 보존하되 누적 체결량과 명확한 종결 상태만 로컬 상태로 축약한다., ReconciliationSummary (+2 more)
...
### Community 359 - "valuation_history"
Cohesion: 0.12
...
### Community 360 - "parse_earnings_release.py"
Cohesion: 0.12
...
Cohesion: 0.07
Nodes (35): normalize_ticker(), 종목 심볼을 대문자 및 표준 dash 형태로 정규화한다., assemble_candidate_features(), CandidateFeatures, CandidateRank, _dense_percentiles(), FactorCandidate, _group_by_ticker() (+27 more)

### Community 363 - "as_date"
Cohesion: 0.18
Nodes (9): MacroDataError, Any, ValueError, as_date(), 날짜로 읽는다. 읽을 수 없으면 `None` — 여기서 예외를 던지면 한 행 때문에 배치 전체가 멈춘다. 값이 없다는 사실은 부르는 쪽이…, ObservationRowTest, 모르는 정밀도를 기본값으로 삼키면 PIT 경계가 조용히 헐거워진다., AsDateTest (+1 more)

### Community 364 - "FactorModel"
Cohesion: 0.20
...
### Community 365 - "DropImplausibleSharesTest"
Cohesion: 0.24
...
### Community 366 - "PositionBaseline"
Cohesion: 0.26
...
### Community 367 - "score_cross_section"
Cohesion: 0.32
...
### Community 370 - "fundamentals/test_repository.py"
Cohesion: 0.20
Nodes (8): _as_of(), datetime, 정정이 원본을 덮지 않고, as-of 조회가 그때의 값을 준다., 정정 공시가 원본을 지우지 않고, cutoff마다 그때 알 수 있던 버전 하나를 고른다., SEC 제출일(6/1)이 지났어도 우리가 받은 것은 6/3이다 — 운영 재현은 원본을 쓴다., 6/1 제출 공시는 6/2 00:00 UTC(뉴욕 6/1 저녁)에는 아직 쓰지 않는다 — 장 마감 뒤 공시일 수 있다., 공시는 받았어도 재처리한 버전 행이 cutoff 뒤에 생겼으면 그때는 몰랐던 값이다., VersionSelectionTest

### Community 371 - "LoadConfigTest"
Cohesion: 0.11
Nodes (10): ConfigError, RuntimeError, 설정이 없거나 모양이 틀렸다. 값 자체는 담지 않는다., 필요한 값을 지금 확인한다. 없으면 **이름만** 말하고 멈춘다., LoadConfigTest, PackageImportTest, 설정은 명시적으로 읽고, 안전 플래그는 fail-closed다., 테스트가 개발자 기계의 .env에 물들지 않아야 한다. (+2 more)
...
### Community 373 - "strategies/db.py"
Cohesion: 0.29
...
### Community 374 - "LocalTradingDatabase"
Cohesion: 0.22
Nodes (3): LocalTradingDatabase, 관계형 판단 원장의 원자 쓰기와 페이지 제한 없는 로컬 조회., Database
...
Cohesion: 0.07
Nodes (13): ColumnDriftTest, _fact(), ManifestTest, PersistedPayloadTest, fundamentals wide 적재의 의미 선택·출처 보존 계약을 검증한다., 정정 공시는 새 버전 행이고, 최신 값은 저장이 아니라 뷰가 고른다., 저장 payload에 스키마에 없는 키가 섞이면 그 공시가 통째로 사라진다., 메모리 전용으로 떼어 내는 키는 스키마가 obsolete로 선언한 것과 같아야 한다. (+5 more)

### Community 387 - "test_market_retention.py"
Cohesion: 0.20
Nodes (4): Market 고정 용량 보존 작업 계약., _Response, _Rpc, _Supabase

### Community 388 - "ShadowFillTest"
Cohesion: 0.27
...
### Community 389 - "datetime"
Cohesion: 0.16
Nodes (9): datetime, 발표 세션(BMO/AMC) 분류·타겟 선정과 발송 상호 배제. 하루 한 번 훑던 시절에는 장전 발표가 최대 15시간 늦게 잡혔고, 로컬…, 정확 시각·추정 시각·날짜만 아는 일정을 서로 다른 창으로 다룬다., 09:30 개장·16:00 마감이 경계다. 실측 JPM 06:00·NVDA 16:00., 시각을 모르면 장전으로 넘겨짚지 않는다 — 넓은 창으로 처리한다., UTC 13:00은 ET 09:00이라 장전이다. 변환 없이 읽으면 장중으로 잘못 본다., SessionClassification, TimedTargetSelection (+1 more)

### Community 390 - "16. 구현 계약"
Cohesion: 0.40
Nodes (5): 16.1 CSS 변수 계약, 16.2 코드 SSOT, 16.3 명명 규칙, 16.4 구현 반영 시 확인할 항목, 16. 구현 계약

### Community 391 - "valuation/engine.py"
Cohesion: 0.19
Nodes (10): build_pit_valuation(), _decimal(), _missing(), PITValuationObservation, _ratio(), PIT 밸류에이션 입력과 관측값의 순수 계산 계약., PIT 입력에서 계산된 저장 후보 관측값이다., 유한한 수치만 Decimal로 정규화한다. (+2 more)
...
### Community 393 - "ApprovalStatusTest"
Cohesion: 0.18
Nodes (6): 한 approval 카드에 되돌려 쓸 broker 관측 상태., ReconciliationCardUpdate, ApprovalStatusTest, consumed_approval(), FakeClient, FakeRepository
...
Cohesion: 0.19
Nodes (7): _count(), _keys(), PostgresNotificationLedger, Any, datetime, 알림 원장의 Supabase 경계. 이 패키지에서 쿼리를 만드는 곳은 여기뿐이다. 판단(예약·전송 시작·결과 기록)은 SQL 함수가 서버…, `NotificationLedger` 계약의 Supabase 구현.
...
Cohesion: 0.11
Nodes (15): _as_datetime(), base_form(), is_amendment(), is_periodic(), normalize_accession(), datetime, SEC 공시 식별자와 종류의 규칙. ## accession_no 하나로 부른다 같은 값을 `accession`, `accessionNo`,…, 정정 표시를 뗀 서식. `10-K/A`와 `10-K`를 같은 종류로 묶을 때 쓴다. (+7 more)

### Community 402 - "number"
Cohesion: 0.32
Nodes (15): build_filing(), _instrument_rows(), _material_changes(), 13F 보고 노출 구성(Equity/CALL/PUT/PRN)을 카드용 행으로 만든다. 각 항목의 비중은 '13F 보고가치' 합 대비다(전체…, _top_holdings(), money(), number(), pct() (+7 more)

### Community 403 - "backfill"
Cohesion: 0.17
Nodes (5): BackfillWindow, backfill(), Any, 시점마다 밸류에이션 → feature 순서로 적재한다. 이미 있는 시점은 건너뛴다., TechIndicatorsRetentionTest
...
Cohesion: 0.07
Nodes (31): finite_float(), Any, 유한한 float만. NaN·Infinity·bool·읽을 수 없는 값은 `default`. bool을 막는 이유: 파이썬에서…, estimate_statistics(), fundamental_statistics(), _gross_profit(), price_statistics(), Any (+23 more)

### Community 406 - "state_versions.py"
Cohesion: 0.20
Nodes (13): _as_datetime(), _normalized(), plan_versions(), datetime, 상태가 바뀔 때만 새 버전을 남기는 판정. 예상치·발표 일정·애널리스트 커버리지가 쓴다. 같은 논리 키(종목·대상 기간·원천 등)의 **직전…, 새로 넣을 버전과 last_seen_at만 옮길 기존 버전을 가른다., same_state(), VersionPlan (+5 more)
...
### Community 409 - "DiscordTargetTests"
Cohesion: 0.21
...
### Community 412 - "trading/test_contracts.py"
Cohesion: 0.40
Nodes (3): _evidence(), EvidenceContractTest, LLM 계약이 미래 근거와 가짜 인용을 차단하는지 검증한다.
...
### Community 414 - "test_factor_research.py"
Cohesion: 0.31
...
### Community 424 - "test_funding_followup.py"
Cohesion: 0.24
Nodes (4): FundingFollowupTest, _intent_row(), 자금 확보 매도가 끝난 System 목표만 한 번 더 따라갈 수 있다., SupersessionTest

### Community 425 - "validate_subscriptions.py"
Cohesion: 0.26
Nodes (8): main(), 알림 채널 환경변수 구성을 읽기 전용으로 검증한다., 각 kind의 Discord 채널 환경변수가 실제로 설정돼 있는지 확인한다., run(), validate(), _config(), workflow용 subscription read-only validation을 검증한다., ValidateSubscriptionsTest
...
### Community 429 - "managers.py"
Cohesion: 0.18
Nodes (11): all_managers(), blind_spot_caveat(), guru_display_name(), guru_thread_topic(), presentation_for(), institutional 7인 레이더의 코드 상수. 추적 대상 manager(cik/name/fund_name/is_active)의 런타임…, 활성 여부와 무관한 전체 manager 사실 + 화면 해석. cik 오름차순., blind_spots 코드 목록을 운용사별 한국어 caveat 한 줄로 만든다. (+3 more)
...
### Community 435 - "test_edgar_parser.py"
Cohesion: 0.11
Nodes (11): SEC EDGAR와 FSDS 외부 데이터 어댑터., ExhibitExtractorTest, ItemClassifierTest, PressReleaseArchiveAdapterTest, 내재화한 SEC 8-K 파서의 표·EX-99·Item 계약을 네트워크 없이 검증한다., _companyfacts_row(), _filing(), CompanyFacts가 덮는 공시는 문서를 받지 않는다. CompanyFacts는 XBRL **차원을 버린다.** 그래서 어떤 공시가… (+3 more)

### Community 436 - "test_native_core.py"
Cohesion: 0.20
Nodes (11): build_tca_report(), _finite(), Any, 주문 결과의 arrival·spread·slippage·implementation shortfall 계산., 원시 가격·체결 결과에서 TCA를 한 번 계산한다., 한 order의 비용을 양수 cost convention으로 남긴다., _side_cost(), TCAReport (+3 more)

### Community 437 - "10_universe.sql"
Cohesion: 0.47
Nodes (3): universe.entities, universe.securities, universe.security_identifiers

### Community 438 - "build_system_portfolio_read_model"
Cohesion: 0.23
Nodes (10): _account_weights(), build_system_portfolio_read_model(), _follow_ratio(), _position_value(), Any, System Portfolio와 My Portfolio를 한 화면에서 비교하는 read model. - System: 목표비중·NAV·기간…, 1 − 회전율 거리. 1이면 System과 같은 비중, 0이면 겹치는 보유가 없다., _system_return_since() (+2 more)
...
Cohesion: 0.29
Nodes (6): _freshness_frame(), Any, DataFrame, News/Social 수집 상태 화면. ## 왜 intelligence.py가 아닌가 `app_pages/intelligence.py`는 이미…, _render_runs(), 저장소별 읽기 경계. 파일 이름이 "어디서 오는가"를 말한다. reporting이 존재하는 이유가 그 질문을 화면과 알림에서 감추는 것이다 —…

### Community 446 - "share_class_snapshots.py"
Cohesion: 0.22
Nodes (12): load_shares_outstanding_by_cik(), Any, datetime, Supabase repository for fundamentals.share_class_snapshots., 한 CIK의 클래스별 발행주식수 시점 기록 전체를 반환한다., 공시일이 cutoff 이전인 발행주식수 snapshot. `accepted_at`이 비어 있는 행도 돌려준다 — 그 경우 호출자가 일자 정밀도…, 한 CIK·filing window를 교체하고 canonical filings와 연결한다., replace_shares_outstanding() (+4 more)

### Community 447 - "PITScalar"
Cohesion: 0.27
...
### Community 456 - "fred_calendar.py"
Cohesion: 0.21
Nodes (11): _fred_slot(), fetch_batch(), fetch_release_dates(), date, FRED 발표 일정(release/dates) 어댑터. macro/clients/fred.py는 관측치(series/observations)를…, release_id 하나의 발표 예정일을 [start, end] 구간으로 잘라 반환한다.…, 릴리스 단위로 한 번씩만 호출한다. 지표 14종이 릴리스 9개를 공유하므로(CPI·Core CPI가 release 10 하나) 지표마다 부르면…, fetch_gdpnow() (+3 more)
...
### Community 459 - "news_normalize.py"
Cohesion: 0.26
...
### Community 460 - "econ_calendar_backfill.py"
Cohesion: 0.15
Nodes (19): _gdpnow_rows(), _historical_own_model_rows(), main(), Any, 명시적 과거 자료 적재. 원자료 빈티지·예상값의 실제 수집시각을 보존한다., 각 first print 직전 정보만 쓴 measure-level own model PIT reconstruction., _release_rows(), _diffs() (+11 more)

### Community 461 - "historical_training_set"
Cohesion: 0.10
Nodes (19): 뒤쪽 구간을 평가용으로 떼어낸다. 학습한 구간에서 채점하면 어떤 정책도 통과한다., split_dataset(), make_purged_walk_forward_splits(), make_walk_forward_splits(), datetime, 학습·검증·OOS 구간이 겹치지 않는 walk-forward 분할., out_of_sample을 일반적인 train/validation/test 용어로 노출한다., 미래 label 구간이 다음 window에 닿는 표본까지 제거한다. (+11 more)
...
Cohesion: 0.06
Nodes (12): universe 수집·정합성 명령 패키지., 무엇을 다룰 것인가 — 회사·증권 identity와 지수 membership. 여기 없는 종목은 수집도 판단도 하지 않는다. 다른 모든…, MembershipReconcileTest, 멤버십이 그대로여도 게이트가 그대로라는 뜻은 아니다. 상류의 거래소 master 동기화가 게이트를 통째로 껐고, 이 경로가 조기 반환하는…, KoreanNameRefreshTest, KoreanNameSelectionTest, Universe 핵심 갱신과 로컬 전용 한글명 보강의 경계 테스트., UniverseWorkflowBoundaryTest (+4 more)
...
### Community 476 - "_LabelRepository"
Cohesion: 0.20
...
### Community 480 - "test_provider.py"
Cohesion: 0.33
...
### Community 486 - "test_shares_outstanding.py"
Cohesion: 0.21
Nodes (5): _match_class_ticker(), 멀티 클래스 멤버와 상장 ticker를 100% 데이터 기반 규칙으로 매핑한다 (기업별/티커별 하드코딩 금지)., _share_row(), ShareEntrypointContractTest, ShareSchemaContractTest
...
### Community 494 - "integrity.py"
Cohesion: 0.20
Nodes (9): 재무지표·XBRL concept·세그먼트 축의 표준 분류 사전., 세그먼트 축 분류에 사용하는 표준 축과 정책 버전., _as_date(), collect_integrity_facts(), _db_columns(), date, 정합성 점검이 읽는 조회와 이슈 기록., PostgREST가 노출하는 canonical financials 컬럼 집합. 한 행만 읽어 키를 본다. information_schema는… (+1 more)
...
### Community 503 - "ReconciliationRepository"
Cohesion: 0.17
Nodes (3): Protocol, `on_breach`는 우리 원장으로 설명되지 않는 계좌 변화에서 신규 주문을 막는 자리다. 알림만 보내면 사람이 보기 전까지 다음 주문이…, ReconciliationRepository

### Community 504 - "resolve_flash_period"
Cohesion: 0.29
...
### Community 505 - "earnings_calendar/candidates.py"
Cohesion: 0.07
Nodes (28): collect(), _default_store(), pending_state(), date, 이번 주 실적 캘린더 후보를 고르는 entry-side reader., 주 1장. 내용은 카드에 찍히는 종목·예정일·서식·신뢰도다 — 바뀌면 그 주 카드를 고친다., 종목·분기마다 "다음 발표 예정". 사람이 아는 예정일과 달라졌을 때만 그 종목 스레드에 남는다., 렌더 의존성 설치 전 사전 점검. 원장이 아직 보내지 않은 주간 카드·예정 안내를 센다. (+20 more)
...
### Community 509 - "earnings_events.py"
Cohesion: 0.25
Nodes (10): load_earnings_results(), load_fiscal_calendar(), Any, 실적 발표 속보 저장을 담당하는 저장소., 실적 속보 행을 자연키 기준으로 멱등 저장한다., 종목의 실제 회계력. 8-K를 올바른 회계분기에 붙이는 유일한 근거다. 달력 월로 분기를 매기면 1월 결산 유통사가 통째로 어긋난다 —…, 수집 게이트를 지난 종목만 CIK별 ticker 목록으로 묶는다. 같은 CIK에는 우선주·채권·자리표시 종목도 있다. 그 ticker로…, 역사 예상치 재구성에 필요한 실적 속보 회계키만 읽는다. 행은 수집 대상 보통주마다 하나다. (+2 more)

### Community 510 - "_collect_news"
Cohesion: 0.24
Nodes (11): 수집 대상 종목. universe가 tracked 종목의 단일 기준이다. CLI와 하네스 잡이 같은 함수를 쓰도록 여기 둔다 — 각자 자기…, watchlist_tickers(), 언급으로 인정할 종목 집합. universe가 모르는 심볼은 언급이 아니다., tracked_tickers(), _collect_news(), _collect_social(), _intelligence_stage(), _prune_intelligence() (+3 more)

### Community 511 - "spearman"
Cohesion: 0.47
...
Cohesion: 0.13
Nodes (7): 운영 점검과 하네스 실행 명령 패키지., DefaultPathsTest, CLI가 기록한 정비 보류와 잠금을 기본 라이브러리도 같은 위치에서 읽는다., LongHorizonCadenceTest, 중장기 보유 전략의 실행 주기: 분석·System 목표 갱신이 분 단위 매매 주기로 되돌아가지 않는다., 공통 Actions 실패 리포터가 원문을 안전한 사건 카드로 바꾸는지 검증한다., WorkflowFailureReporterTest
...
### Community 522 - "investment/db.py"
Cohesion: 0.22
Nodes (10): latest_analysis_run_id(), latest_portfolio(), Any, 자동매매 보고서의 로컬 판단·실행 원장 조회 경계., 가장 최근 System Portfolio 목표와 그 risk 판정·실행을 로컬 원장에서 묶는다. 실계좌 추종…, 종목 판단이 기록된 가장 최근 분석 회차. System 목표 회차와 달리 종목 논지를 갖는다., 한 실행의 완료 판단을 신뢰도 순으로 상위 N개 돌려준다., 지정 시각 이후 갱신된 로컬 주문과 체결·실행 대상을 모은다. (+2 more)

### Community 523 - "graphify reference: query, path, explain"
Cohesion: 0.33
Nodes (5): For /graphify explain, For /graphify path, graphify reference: query, path, explain, Step 0 — Constrained query expansion (REQUIRED before traversal), Step 1 — Traversal

### Community 524 - "macro_indicator_rows"
Cohesion: 0.23
...
Cohesion: 0.13
Nodes (12): audit_fx_cross_sources(), _comparison(), date, Series, 매크로 원천 시계열의 단위·범위와 환율 교차검증을 담당한다., BOK 환율을 독립적인 연준/FRED 관측치와 교차검증한다. ECOS를 주 원천으로 유지한다. FRED의 DEXKOUS(원/달러)와…, 지표별 수집 계약을 확인하고 명백한 단위·스파이크 오류를 차단한다. ``source_params.validation``은 원천값을 화면 단위로…, validate_series() (+4 more)
...
Cohesion: 0.19
Nodes (6): LoadReleasedTest, datetime, 경제 발표 알림 — 발표 하나가 묶음 전송과 반복 실행을 거쳐도 사람에게 한 번 닿는다., ReleaseNoticeTest, ReleasePublishTest, _row()

### Community 529 - "GuruNotificationTest"
Cohesion: 0.44
Nodes (3): _filing(), GuruNotificationTest, 거장 13F 알림 — 제출은 사람 스레드에 한 번, 분기 요약은 한 장을 고쳐 가며 닿는다.

### Community 530 - "src/investment_agent/data/macro/releases/__init__.py"
Cohesion: 0.15
Nodes (5): Macro application orchestration boundary., 지표 발표 캘린더에서 공통으로 쓰는 설정값., ECON release-time watcher의 due/filter/pending/idempotency 계약., _release(), ReleaseWatchTest

### Community 531 - "download_monthly_close"
Cohesion: 0.22
...
### Community 547 - "first_print"
Cohesion: 0.24
Nodes (6): first_print(), 최초 발표에서 최종값까지의 변화량. 정정이 없었으면 `None`. 크게 고쳐지는 지표는 최초 발표를 그대로 믿으면 안 된다는 신호다., 최초 발표값. 정정 전 숫자라 서프라이즈 계산의 기준이 된다., revision_size(), FirstPrintTest, RevisionSizeTest
...
### Community 554 - "AlfredParserTest"
Cohesion: 0.22
...
### Community 557 - "test_operational_guardrails.py"
Cohesion: 0.22
Nodes (3): EmptySourceTest, LazySupabaseClientTest, 권한·빈 응답·지연 DB 초기화 같은 운영 안전장치 회귀 테스트.
...
### Community 576 - "ManifestDriftTest"
Cohesion: 0.25
Nodes (4): ManifestDriftTest, `env`(시크릿) 아니면 `name`(봇이 길드에서 찾음) 중 하나는 있어야 한다., 여기만 따로 두면 채널을 옮길 때 한쪽만 고치게 된다., 감시 목록에서 빠진 채널은 죽어도 아무도 모른다. 랩은 제외한다.
...
### Community 580 - ".macro_histories"
Cohesion: 0.22
Nodes (5): date, 사후 평가용 경로. PIT 조회가 아니다., 연구 전용 횡단면 종가 — evidence/feature 경로에서 부르지 않는다(미래 가격일 수 있다)., market 스키마의 owner에게 위임한다., series별 판단 시점까지 알려진 (관측일, 값) 이력. 노출 규칙이 수준과 변화폭을 함께 본다.

### Community 581 - "BoundaryTest"
Cohesion: 0.33
Nodes (4): BoundaryTest, Path, System과 ALPHA는 실계좌·승인·주문 코드를 import하지 않는다., 검사 대상이 사라지면 아래 검사는 공허하게 통과한다.

### Community 583 - "redact"
Cohesion: 0.32
Nodes (4): LogRecord, 알려진 비밀값 모양을 가린다. 값의 길이도 남기지 않는다., redact(), RedactTest

### Community 585 - "._tick_with_fakes"
Cohesion: 0.27
...
### Community 587 - "IcSummary"
Cohesion: 0.43
Nodes (5): IcSummary, Any, 평균 IC가 양수이고 겹침 보정 t가 기준 이상인 category만 IC 비례 가중치. 없으면 동일가중 유지., suggest_category_weights(), SummaryTest

### Community 590 - "src/investment_agent/data/market/__init__.py"
Cohesion: 0.22
Nodes (5): 거래일 단위 시세와 corporate action. **관측한 것만 담는다.** 조정가·수익률처럼 계산으로 나오는 값은 저장하지 않고…, GuardrailTest, Phase 1에 주문 표면이 생기지 않고 DB가 비공개인지 검증한다., trading은 Supabase 스키마가 아니라 실행 컴퓨터 로컬 runtime.sqlite3가 소유한다 — PostgREST에 노출되지 않는…, 벤치마크는 시세만 들고 있는 대상이다 — 판단 유니버스에 섞이지 않는다. 목록을 정확한 튜플로 못박으면 벤치마크를 하나 늘릴 때마다 이 검사가…
...
### Community 603 - "RunLifecycleTest"
Cohesion: 0.25
...
### Community 610 - "latest_cross_section"
Cohesion: 0.53
Nodes (3): latest_cross_section(), 판단 시각이 가장 최근이면서 종목 수가 `min_coverage` 이상인 한 시점의 feature 횡단면. feature 적재가 중간에 끊긴…, LatestCrossSectionTest

### Community 628 - "DashboardStaticBoundaryTests"
Cohesion: 0.29
...
### Community 638 - "actions_budget.py"
Cohesion: 0.43
...
### Community 641 - "WebClientError"
Cohesion: 0.29
Nodes (6): Exception, RuntimeError, 웹 원천 수집 계약 위반의 공통 기반 예외., 외부 페이지 호출 또는 응답 파싱이 실패했다., WebClientError, WebProviderError

### Community 644 - "CountersTest"
Cohesion: 0.33
Nodes (3): CountersTest, 원장이 Supabase에 있으므로 GitHub runner도 로컬과 같은 대기 수를 읽는다., 카운터 하나가 실패했다고 점검이 사라지면 점검이 없는 것보다 나쁘다.

### Community 648 - "test_watchlists.py"
Cohesion: 0.29
Nodes (3): 알림용 reporting read model과 adapter 패키지., Supabase 관심종목 관리와 공시 필터 규칙을 검증한다., WatchlistNormalizationTest

### Community 661 - "src/investment_agent/trading/decision/__init__.py"
Cohesion: 0.29
Nodes (4): 판단 하나를 가리키는 키와 재현 지문. 키를 계산해서 만드는 이유는 같은 판단이 두 행으로 갈라지지 않게 하기 위해서다., AnalysisPathTest, RL 정책은 ALPHA 분석 경로에 섞이지 않는다. RL은 오프라인 연구·평가로만 남는다., 분석은 논지만 남긴다. RL 목표비중·포트폴리오 제안을 다시 만들지 않게 한다.

### Community 662 - "SurpriseViewContractTest"
Cohesion: 0.33
Nodes (3): 비교 뷰가 발표 뒤에 만든 값이나 다른 단위를 서프라이즈에 쓰지 않는다., 발표 전 예상 이력은 다시 받을 수 없다. 버전 저장이라 지울 반복도 없다., SurpriseViewContractTest
...
### Community 671 - "HarnessModuleAllowlistTest"
Cohesion: 0.40
...
### Community 674 - "PPOAllocationTimingSpec"
Cohesion: 0.40
Nodes (3): PPOAllocationTimingSpec, Any, PPO가 broker나 종목 수량을 직접 만들지 않는 연구 명세.

## Knowledge Gaps
- **598 isolated node(s):** `content_files`, `content_catalog_state`, `feature_sets`, `dataset_runs`, `notifications.threads` (+593 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 5897 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **110 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `get_logger()` connect `logging.py` to `web.py`, `SupabaseRepository`, `macro/core.py`, `ExecutionIntent`, `investment/db.py`, `companyfacts.py`, `watch_earnings.py`, `investment_harness.py`, `reserve_provider_call`, `readers/intelligence.py`, `continuous_retrain.py`, `reconcile_toss.py`, `TransactionCostModel`, `select_all_paged`, `notify_ops`, `MarketRepository`, `target.py`, `rl/contracts.py`, `yahoo.py`, `system/engine.py`, `application/refresh_expectations.py`, `universe/persistence.py`, `toss/client.py`, `factor_research.py`, `tradingagents_adapter.py`, `features/db.py`, `parse_shares.py`, `application/backfill_history.py`, `build_labels.py`, `risk/gate.py`, `SegmentMetricRepository`, `Database`, `decision/analysis.py`, `ensure_aware`, `supabase/segment_metrics.py`, `release_calendar.py`, `load_config`, `retry.py`, `parser.py`, `UniverseRepository`, `budget.py`, `sec_entities.py`, `infrastructure/sources/yfinance.py`, `Filing13F`, `ml_challengers.py`, `fsds.py`, `strategies.py`, `collection.py`, `directory.py`, `strategies/etl.py`, `request_toss_approval.py`, `create_execution_intent.py`, `company_financials.py`, `src/investment_agent/data/fundamentals/application/__init__.py`, `TossAuthError`, `event_reanalysis.py`, `alfred.py`, `application/etl.py`, `openfigi.py`, `main`, `process_filing.py`, `trading/repository.py`, `backtest/cli.py`, `refresh_earnings_season.py`, `build_decision_experiences.py`, `map_fiscal_periods.py`, `portfolio/contracts.py`, `postgres.py`, `harness/runtime.py`, `discord_admin/client.py`, `adopt_ml_model.py`, `refresh_market_state.py`, `harness_adapters.py`, `monitoring/discord.py`, `notifications/macro.py`, `remap_runtime_securities.py`, `validate_subscriptions.py`, `share_class_snapshots.py`, `actuals.py`, `fred_calendar.py`, `news_normalize.py`, `econ_calendar_backfill.py`, `integrity.py`, `earnings_calendar/candidates.py`?**
  _High betweenness centrality (0.154) - this node is a cross-community bridge._
- **Why does `parse_datetime()` connect `parse_datetime` to `build_research_dataset`, `JsonStateStore`, `EvidenceBundle`, `SupabaseRepository`, `live_worker.py`, `update_performance`, `ExecutionIntent`, `build_decision_experiences.py`, `ml_challengers.py`, `fit_baseline`, `canonical_json`, `valuation/engine.py`, `evidence/artifacts.py`, `market_schedule.py`, `logging.py`, `filings.py`, `serialization.py`, `portfolio/contracts.py`, `_date`, `continuous_retrain.py`, `reconcile_toss.py`, `TransactionCostModel`, `select_all_paged`, `json_value`, `context.py`, `FeatureDataset`, `request_toss_approval.py`, `ProductionInvestmentAdapters`, `target.py`, `event_intelligence.py`, `build_features`, `MarketQuote`, `ContractError`, `ResearchDataset`, `harness/runtime.py`, `test_native_core.py`, `rl/contracts.py`, `create_execution_intent.py`, `validated_weights`, `layer.py`, `VersionKey`, `system/engine.py`, `inputs.py`, `toss/client.py`, `event_reanalysis.py`, `harness_adapters.py`, `OptimizerPolicy`, `tradingagents_adapter.py`, `FakeRepository`, `InvestmentDossier`, `test_dataset_export.py`, `purged_row_splits`, `HarnessScheduler`, `build_labels.py`, `.create`, `build_market_regime`, `risk/gate.py`, `_LabelRepository`, `decision/analysis.py`, `ensure_aware`, `worker.py`, `candidate_ranker.py`, `as_date`, `build_events`, `TradingAgentsAdapterTest`, `promotion/gate.py`, `LocalEvidenceCache`, `PITValuationInputs`, `fusion.py`, `BacktestRequest`, `parse_external_payload`, `services/investment/__init__.py`, `app_pages/intelligence.py`?**
  _High betweenness centrality (0.055) - this node is a cross-community bridge._
- **Why does `canonical_json()` connect `canonical_json` to `build_research_dataset`, `EvidenceBundle`, `SupabaseRepository`, `live_worker.py`, `ml_challengers.py`, `build_decision_experiences.py`, `valuation/engine.py`, `fit_baseline`, `evidence/artifacts.py`, `emergency.py`, `investment_harness.py`, `logging.py`, `parse_datetime`, `serialization.py`, `finite_float`, `market_risk.py`, `switch.py`, `remap_runtime_securities.py`, `continuous_retrain.py`, `TransactionCostModel`, `json_value`, `context.py`, `FeatureDataset`, `request_toss_approval.py`, `ProductionInvestmentAdapters`, `event_intelligence.py`, `dashboard/db.py`, `CaseMemory`, `MarketQuote`, `ContractError`, `harness/runtime.py`, `test_native_core.py`, `lifecycle.py`, `install_investment_harness.py`, `LocalArtifactStore`, `rl/contracts.py`, `platform/artifacts.py`, `layer.py`, `content_hash`, `system/engine.py`, `keys.py`, `event_reanalysis.py`, `OptimizerPolicy`, `tradingagents_adapter.py`, `InvestmentDossier`, `backtest/contracts.py`, `DiscordChannel`, `HarnessScheduler`, `risk/gate.py`, `decision/analysis.py`, `load_config`, `worker.py`, `PortfolioRiskPolicy`, `ResearchStore`, `LocalEvidenceCache`, `PITValuationInputs`, `FinRLTrainer`, `services/investment/__init__.py`, `harness/contracts.py`?**
  _High betweenness centrality (0.031) - this node is a cross-community bridge._
- **Are the 30 inferred relationships involving `parse_datetime()` (e.g. with `._validate_timing()` and `build_events()`) actually correct?**
  _`parse_datetime()` has 30 INFERRED edges - model-reasoned connections that need verification._
- **Are the 55 inferred relationships involving `FakeDatabase` (e.g. with `ConsensusTest` and `ProcessingTest`) actually correct?**
  _`FakeDatabase` has 55 INFERRED edges - model-reasoned connections that need verification._
- **Are the 21 inferred relationships involving `SupabaseRepository` (e.g. with `run()` and `ProductionInvestmentAdapters`) actually correct?**
  _`SupabaseRepository` has 21 INFERRED edges - model-reasoned connections that need verification._
